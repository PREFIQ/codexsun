...
export class MasterRepository {
  ...
  async create(tenantId: string, input: MasterSaveInput) { const record = normalizeRecord(tenantId, input, null, this.definition.key); await getCoreDatabase().insertInto(this.definition.tableName as never).values(toRow(record) as never).execute(); await syncChildTables(this.definition.tableName, record); return record; }
  async update(tenantId: string, id: string, input: MasterSaveInput) { const current = await this.find(tenantId, id); if (!current || current.tenantId !== tenantId || isReserved(current)) return null; const record = normalizeRecord(tenantId, input, current, this.definition.key); await getCoreDatabase().updateTable(this.definition.tableName as never).set(toRow(record) as never).where("tenant_id" as never, "=", tenantId as never).where("id" as never, "=", id as never).execute(); await syncChildTables(this.definition.tableName, record); return record; }
