  ...
  private async saveEntry(kind: EntryKind, tenantId: string, input: EntryUpsertInput, existingId: string | null) {
    ...
    if (existing) {
      await sql`
        ...
        SET document_no = ${normalized.documentNo}, document_date = ${normalized.documentDate}, customer_id = ${normalized.customerId},
          ...
          work_order_no = ${normalized.workOrderNo}, payment_term_id = ${normalized.paymentTermId}, payment_term_name = ${normalized.paymentTermName},
      ...
      `.execute(getCoreDatabase());
      await sql`DELETE FROM ${sql.table(itemTableName(kind))} WHERE ${sql.ref(`${kind}_entry_id`)} = ${existing.id}`.execute(getCoreDatabase());
    ...
    await sql`
      INSERT INTO ${sql.table(tableName(kind))} (
        ...
        due_date, work_order_no, payment_term_id, payment_term_name, sales_type_id, sales_type_name,
        ...
        ${normalized.dueDate}, ${normalized.workOrderNo}, ${normalized.paymentTermId}, ${normalized.paymentTermName},
...
function normalizeEntryInput(kind: EntryKind, tenantId: string, input: EntryUpsertInput, existing: EntryRecord | null) {
  ...
  return {
    ...
    customerName: cleanRequired(input.customerName, "Customer name"),
...
function labelFor(kind: EntryKind) {
  return kind === "quotation" ? "Quotation" : "Sales";
}

function nextDocumentNo(kind: EntryKind, tenantId: string) {
  const prefix = kind === "quotation" ? "QUO" : "SAL";
