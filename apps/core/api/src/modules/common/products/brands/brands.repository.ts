...
export class BrandsRepository {
  async list(filters: BrandsListFilters = {}) {
    const rows = await sql<BrandsRow>`SELECT id, name, status, sort_order FROM brands
  ...
  async create(input: BrandsSavePayload) {
    const result = await sql`INSERT INTO brands (name, status, sort_order) VALUES
  ...
  async update(id: string | number, input: BrandsSavePayload) {
    ...
    await sql`UPDATE brands SET name=${normalizeString(input.name)}, status=${input.isActive === false ? "inactive" : "active"},
  ...
  async setActive(id: string | number, isActive: boolean) {
    const existing = await this.find(id);
    if (!existing || !canMutate(existing)) return null;
    await sql`UPDATE brands SET status=${isActive ? "active" : "inactive"}, updated_at=CURRENT_TIMESTAMP WHERE id=${Number(id)}`.execute(
      getCoreDatabase()
    );
    return this.find(id);
  }
  ...
  async forceDelete(id: string | number) {
    const existing = await this.find(id);
    if (!existing || !canMutate(existing)) return null;
    await sql`DELETE FROM brands WHERE id=${Number(id)}`.execute(getCoreDatabase());
    return existing;
  }
}
