...
export class BrandsRepository {
  async list(filters: BrandsListFilters = {}) {
    const rows = await sql<BrandsRow>`SELECT id, uuid, name, is_active, sort_order FROM brands
  ...
  async create(input: BrandsSavePayload) {
    const id = `brands-${randomUUID()}`;
    const uuid = randomUUID().replaceAll("-", "").slice(0, 8);
    await sql`INSERT INTO brands (id, uuid, name, is_active, sort_order) VALUES
  ...
  async update(id: string, input: BrandsSavePayload) {
    ...
    await sql`UPDATE brands SET name=${normalizeString(input.name)}, is_active=${input.isActive === false ? 0 : 1},
  ...
  async setActive(id: string, isActive: boolean) {
    const existing = await this.find(id);
    if (!existing || !canMutate(existing)) return null;
    await sql`UPDATE brands SET is_active=${isActive ? 1 : 0}, updated_at=CURRENT_TIMESTAMP WHERE id=${id}`.execute(
      getCoreDatabase()
    );
    return this.find(id);
  }
  ...
  async forceDelete(id: string) {
    const existing = await this.find(id);
    if (!existing || !canMutate(existing)) return null;
    await sql`DELETE FROM brands WHERE id=${id}`.execute(getCoreDatabase());
    return existing;
  }
}
