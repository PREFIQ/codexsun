...
export class WarehousesRepository {
  async list(filters: WarehousesListFilters = {}) {
    const rows =
      await sql<WarehousesRow>`SELECT id, uuid, name, is_active, sort_order FROM warehouses
  ...
  async create(input: WarehousesSavePayload) {
    const id = `warehouses-${randomUUID()}`;
    const uuid = randomUUID().replaceAll("-", "").slice(0, 8);
    await sql`INSERT INTO warehouses (id, uuid, name, is_active, sort_order) VALUES
  ...
  async update(id: string, input: WarehousesSavePayload) {
    ...
    await sql`UPDATE warehouses SET name=${normalizeString(input.name)}, is_active=${input.isActive === false ? 0 : 1},
  ...
  async setActive(id: string, isActive: boolean) {
    const existing = await this.find(id);
    if (!existing || !canMutate(existing)) return null;
    await sql`UPDATE warehouses SET is_active=${isActive ? 1 : 0}, updated_at=CURRENT_TIMESTAMP WHERE id=${id}`.execute(
      getCoreDatabase()
    );
    return this.find(id);
  }
  ...
  async forceDelete(id: string) {
    const existing = await this.find(id);
    if (!existing || !canMutate(existing)) return null;
    await sql`DELETE FROM warehouses WHERE id=${id}`.execute(getCoreDatabase());
    return existing;
  }
}
