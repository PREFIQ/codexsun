...
export class CityRepository {
  async list(filters: CityListFilters = {}) {
    const rows = await sql<CityRow>`SELECT cities.id, cities.uuid, cities.district_id,
  ...
  async districtExists(districtId: string) {
  ...
  async create(input: CitySavePayload) {
    const result = await sql`INSERT INTO cities (uuid, district_id, name, sort_order, status) VALUES
      (${randomBytes(4).toString("hex")}, ${Number(input.districtId)}, ${input.name}, ${input.sortOrder}, ${input.status})`.execute(
      getCoreDatabase()
    );
