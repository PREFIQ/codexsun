...
export async function migrateCompanyModule(database: Kysely<CoreDatabase>) {
  await sql
    .raw(
      `CREATE TABLE IF NOT EXISTS companies (id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,code VARCHAR(80) NOT NULL,name VARCHAR(191) NOT NULL,legal_name VARCHAR(191) NULL,type_id INT NULL,type_name VARCHAR(191) NULL,group_id INT NULL,group_name VARCHAR(191) NULL,primary_phone VARCHAR(80) NULL,primary_email VARCHAR(191) NULL,gstin VARCHAR(40) NULL,pan VARCHAR(40) NULL,msme_no VARCHAR(80) NULL,msme_category VARCHAR(80) NULL,tan_no VARCHAR(80) NULL,tds_available TINYINT(1) NOT NULL DEFAULT 0,tcs_available TINYINT(1) NOT NULL DEFAULT 0,opening_balance DOUBLE NOT NULL DEFAULT 0,credit_limit DOUBLE NOT NULL DEFAULT 0,website VARCHAR(255) NULL,description TEXT NULL,logo_path VARCHAR(255) NULL,logo_dark_path VARCHAR(255) NULL,industry_id INT NULL,industry_name VARCHAR(191) NULL,status VARCHAR(24) NOT NULL DEFAULT 'active',emails_json JSON NULL,phones_json JSON NULL,addresses_json JSON NULL,bank_accounts_json JSON NULL,social_links_json JSON NULL,created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,UNIQUE KEY companies_code_unique(code),INDEX companies_status_name(status,name)) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci`
    )
    .execute(database);
