...
export function CommonMasterWorkspace({ definition }: { definition: CommonMasterDefinition }) {
  ...
  const [deleting, setDeleting] = useState<CommonMasterRecord | null>(null);
  ...
  const forceDelete = useMutation({
    mutationFn: (record: CommonMasterRecord) => forceDeleteCommonMaster(definition.path, record.id),
    onSuccess: async (record) => {
      await queryClient.invalidateQueries({ queryKey: ["core", "common", definition.key] });
      toast.success(`${singular(definition.label)} force deleted`, { description: `${String(record.name ?? "Record")} was permanently removed.` });
      setDeleting(null);
    },
    onError: (error) => toast.error(`Unable to delete ${singular(definition.label).toLowerCase()}`, {
  ...
  return (
    ...
    >
      <WorkspaceFilters
        ...
        filterValue={status}
        onFilterValueChange={(value) => { setStatus(value); setPage(1); }}
      ...
      <AlertDialog open={deleting !== null} onOpenChange={(open) => !open && setDeleting(null)}>
        <AlertDialogContent className="rounded-md">
          <AlertDialogHeader>
            <AlertDialogTitle>Force delete {singular(definition.label).toLowerCase()}?</AlertDialogTitle>
            <AlertDialogDescription>
              {String(deleting?.name ?? "This record")} will be permanently removed. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
export function CommonMasterList({ definition, loading, onEdit, onForceDelete, onToggle, records, startIndex = 0 }: {
  definition: CommonMasterDefinition; loading: boolean; onEdit: (record: CommonMasterRecord) => void;
  onForceDelete?: (record: CommonMasterRecord) => void; onToggle: (record: CommonMasterRecord) => void;
  records: CommonMasterRecord[]; startIndex?: number;
}) {
  const finalized = isFinalizedContactMaster(definition);
  return <WorkspaceTablePanel><div className="overflow-x-auto"><table className="w-full min-w-[680px] text-sm">
    ...
    <tbody>{records.map((record, index) => {
      ...
      const editable = !placeholder && (record.tenantId !== "global" || finalized);
      return <tr className="border-b border-border/70 last:border-0" key={record.id}>
      <td className="px-4 py-2.5 text-muted-foreground">{startIndex + index + 1}</td>
      {definition.fields.map((field, fieldIndex) => <td className="px-4 py-2.5" key={field.key}>
        {placeholder && fieldIndex === 0 ? "-" : fieldIndex === 0 && editable ? (
          <button className="cursor-pointer text-left font-medium hover:underline" onClick={() => onEdit(record)} type="button">
      ...
      <td className="px-4 py-1.5 text-right">{placeholder ? <WorkspaceProtectedIndicator /> : !editable ? <span className="text-xs text-muted-foreground">Shared</span> :
        <WorkspaceRowActions
  ...
  return <WorkspaceUpsertDialog className="max-h-[90vh] overflow-y-auto sm:max-w-xl" description={`Enter the ${singular(definition.label).toLowerCase()} details.`} open={open} onClose={onClose} title={`${record ? "Edit" : "New"} ${singular(definition.label).toLowerCase()}`}>
    <form key={`${definition.key}:${record?.id ?? "new"}:${open}`} onSubmit={(event) => { event.preventDefault(); onSubmit(value); }}>
      ...
      <WorkspaceFormGrid columns={1}>
        ...
        {!finalized ? <WorkspaceFormField label="Sort order"><Input min={0} type="number" value={String(value.sortOrder ?? 1000)} onChange={(event) => setValue((current) => ({ ...current, sortOrder: Number(event.target.value) }))} /></WorkspaceFormField> : null}
        {finalized ? (
          <div className={cn(
            "flex h-11 items-center gap-2 rounded-md border px-3",
            value.isActive ? "border-emerald-300 bg-emerald-50 text-emerald-800" : "border-border/80 bg-muted/30 text-muted-foreground"
          )}>
            <CheckCircle2 className="size-4 shrink-0" />
            <span className="text-sm font-medium">Active</span>
            <Switch aria-label="Active" checked={Boolean(value.isActive)} className="ml-auto" onCheckedChange={(checked) => setValue((current) => ({ ...current, isActive: checked }))} />
          </div>
        ) : (
...
function isFinalizedContactMaster(definition: CommonMasterDefinition) {
  return FINALIZED_COMMON_MASTER_KEYS.has(definition.key);
}
function isPlaceholder(record: CommonMasterRecord, definition: CommonMasterDefinition) {
  return definition.fields.some((field) => String(record[field.key] ?? "").trim() === "-");
}

const FINALIZED_COMMON_MASTER_KEYS = new Set([
  "contactGroups", "contactTypes", "addressTypes", "bankNames",
  "productGroups", "productCategories", "productTypes", "units", "hsnCodes",
  "taxes", "brands", "colours", "sizes", "styles",
  "workOrderTypes", "transports", "warehouses", "destinations", "stockRejectionTypes",
  "currencies", "priorities", "paymentTerms", "salesTypes", "months"
]);
