...
import { pincodeDefinitions } from "./pincode.types";
import {
  createPincodeRecord,
  forceDeletePincodeRecord,
  listPincodeRecords,
  suspendPincodeRecord,
  updatePincodeRecord
...
export function PincodeModuleShell({ kind }: { kind: PincodeKind }) {
  const definition = pincodeDefinitions[kind];
  ...
  const recordsQuery = useQuery({
    ...
    queryFn: () => listPincodeRecords(definition.path)
  });
  const parentQueries = usePincodeParents(kind);
  const mutation = useMutation({
    mutationFn: (payload: PincodeSavePayload) =>
      editing
        ? updatePincodeRecord(definition.path, editing.id, payload)
        : createPincodeRecord(definition.path, payload),
  ...
  const rowActionMutation = useMutation({
    mutationFn: async ({ record, type }: { record: PincodeRecord; type: "delete" | "suspend" }) =>
      type === "delete"
        ? forceDeletePincodeRecord(definition.path, record.id)
        : suspendPincodeRecord(definition.path, record.id),
...
export function PincodeTable({
  ...
  definition: PincodeDefinition;
  loading: boolean;
  onEdit: (record: PincodeRecord) => void;
  onForceDelete?: (record: PincodeRecord) => void;
  onSuspend?: (record: PincodeRecord) => void;
  onView?: (record: PincodeRecord) => void;
        ...
        <table className="w-full min-w-[760px] border-collapse text-sm">
          ...
          <tbody>
            {records.map((record, index) => {
              const canMutate = canMutatePincodeRow(definition.kind, record);
              const protectedRow = isProtectedPincodeRow(definition.kind, record);
              return (
                  ...
                  {definition.columns.map((column) => (
                    ...
                    >
                      {column.key === "status" ? (
                        ...
                        />
                      ) : isPrimaryPincodeColumn(definition.kind, column.key) && canMutate ? (
                        <button
...
function canMutatePincodeRow(kind: PincodeKind, record: PincodeRecord) {
  if (isProtectedPincodeRow(kind, record)) return false;
  return true;
}

function isProtectedPincodeRow(kind: PincodeKind, record: PincodeRecord) {
...
function isPrimaryPincodeColumn(kind: PincodeKind, key: keyof PincodeRecord) {
  return key === "name" || (kind === "pincode" && key === "pincode");
}

type PincodeParents = {
  cities: PincodeRecord[];
  countries: PincodeRecord[];
  districts: PincodeRecord[];
  states: PincodeRecord[];
};

function usePincodeParents(kind: PincodeKind): PincodeParents {
  const countries = useQuery({
    enabled: kind === "state" || kind === "district" || kind === "city" || kind === "pincode",
    queryKey: ["core", "location", "country"],
    queryFn: () => listPincodeRecords(pincodeDefinitions.country.path)
  });
  const states = useQuery({
    enabled: kind === "district" || kind === "city" || kind === "pincode",
    queryKey: ["core", "location", "state"],
    queryFn: () => listPincodeRecords(pincodeDefinitions.state.path)
  });
  const districts = useQuery({
    enabled: kind === "city" || kind === "pincode",
    queryKey: ["core", "location", "district"],
    queryFn: () => listPincodeRecords(pincodeDefinitions.district.path)
  });
  const cities = useQuery({
    enabled: kind === "pincode",
    queryKey: ["core", "location", "city"],
    queryFn: () => listPincodeRecords(pincodeDefinitions.city.path)
  });
  return {
    cities: cities.data ?? [],
...
export function PincodeUpsertDialog({
  definition,
  error,
  loading,
  onClose,
  ...
  loading: boolean;
  onClose: () => void;
  onSubmit: (payload: PincodeSavePayload) => void;
  open: boolean;
  parents: PincodeParents;
...
function PincodeForm({
  ...
  definition: PincodeDefinition;
  error: string;
  initialValue: PincodeSavePayload;
  loading: boolean;
  onCancel: () => void;
  onSubmit: (payload: PincodeSavePayload) => void;
  parents: PincodeParents;
  ...
  const update = (patch: Partial<PincodeSavePayload>) => {
  ...
  const createParent = async (
    ...
    const payload = inlineCreatePincodePayload();
    ...
    try {
      const created = await createPincodeRecord(pincodeDefinitions[kind].path, payload);
      await queryClient.invalidateQueries({ queryKey: ["core", "location", kind] });
      toast.success(`${pincodeDefinitions[kind].label} created`, {
        description: `${created.name} was selected.`
      });
      return { label: created.name, value: String(created.id) };
    } catch (error) {
      toast.error(`Unable to create ${pincodeDefinitions[kind].label.toLowerCase()}`, {
        description: error instanceof Error ? error.message : "Please try again."
      });
      return undefined;
    }
...
function toFormValue(kind: PincodeKind, record: PincodeRecord | null): PincodeSavePayload {
  if (record) {
    const { id: _id, ...value } = record;
    return value;
  }
  return emptyPincodePayload(kind);
}

function emptyPincodePayload(kind?: PincodeKind): PincodeSavePayload {
  return {
    areaName: null,
    capital: null,
    cityId: null,
...
function inlineCreatePincodePayload(): PincodeSavePayload {
  return {
    ...emptyPincodePayload(),
    capital: "-",
    currencyCode: "-",
...
export function PincodeWorkspace() {
  return <PincodeModuleShell kind="pincode" />;
}
