...
import { stateDefinitions } from "./state.types";
import {
  createStateRecord,
  forceDeleteStateRecord,
  listStateRecords,
  suspendStateRecord,
  updateStateRecord
} from "./state.services";
import type { StateDefinition, StateKind, StateRecord, StateSavePayload } from "./state.types";

export function StateModuleShell({ kind }: { kind: StateKind }) {
  const definition = stateDefinitions[kind];
  ...
  const [viewing, setViewing] = useState<StateRecord | null>(null);
  ...
  const recordsQuery = useQuery({
    ...
    queryFn: () => listStateRecords(definition.path)
  });
  const parentQueries = useStateParents(kind);
  const mutation = useMutation({
    mutationFn: (payload: StateSavePayload) =>
      editing
        ? updateStateRecord(definition.path, editing.id, payload)
        : createStateRecord(definition.path, payload),
  ...
  const rowActionMutation = useMutation({
    mutationFn: async ({ record, type }: { record: StateRecord; type: "delete" | "suspend" }) =>
      type === "delete"
        ? forceDeleteStateRecord(definition.path, record.id)
        : suspendStateRecord(definition.path, record.id),
  ...
  return (
    ...
    >
      ...
      />
      <StateViewDialog definition={definition} onClose={() => setViewing(null)} record={viewing} />
      <AlertDialog
        ...
        <table className="w-full min-w-[760px] border-collapse text-sm">
          ...
          <tbody>
            {records.map((record, index) => {
              const canMutate = canMutateStateRow(definition.kind, record);
              const protectedRow = isProtectedStateRow(definition.kind, record);
              return (
                  ...
                  {definition.columns.map((column) => (
                    ...
                    >
                      {column.key === "status" ? (
                        ...
                        />
                      ) : isPrimaryStateColumn(definition.kind, column.key) && canMutate ? (
                        <button
...
function canMutateStateRow(kind: StateKind, record: StateRecord) {
  if (isProtectedStateRow(kind, record)) return false;
  return true;
}

function isProtectedStateRow(kind: StateKind, record: StateRecord) {
...
function isPrimaryStateColumn(kind: StateKind, key: keyof StateRecord) {
  return key === "name" || (kind === "pincode" && key === "pincode");
}

type StateParents = {
  cities: StateRecord[];
  countries: StateRecord[];
  districts: StateRecord[];
  states: StateRecord[];
};

function useStateParents(kind: StateKind): StateParents {
  const countries = useQuery({
    enabled: kind === "state" || kind === "district" || kind === "city" || kind === "pincode",
    queryKey: ["core", "location", "country"],
    queryFn: () => listStateRecords(stateDefinitions.country.path)
  });
  const states = useQuery({
    enabled: kind === "district" || kind === "city" || kind === "pincode",
    queryKey: ["core", "location", "state"],
    queryFn: () => listStateRecords(stateDefinitions.state.path)
  });
  const districts = useQuery({
    enabled: kind === "city" || kind === "pincode",
    queryKey: ["core", "location", "district"],
    queryFn: () => listStateRecords(stateDefinitions.district.path)
  });
  const cities = useQuery({
    enabled: kind === "pincode",
    queryKey: ["core", "location", "city"],
    queryFn: () => listStateRecords(stateDefinitions.city.path)
  });
  return {
    cities: cities.data ?? [],
...
export function StateUpsertDialog({
  definition,
  error,
  loading,
  onClose,
...
function StateForm({
  ...
  const update = (patch: Partial<StateSavePayload>) => {
  ...
  const createParent = async (
    ...
    const payload = inlineCreateStatePayload();
    ...
    try {
      const created = await createStateRecord(stateDefinitions[kind].path, payload);
      await queryClient.invalidateQueries({ queryKey: ["core", "location", kind] });
      toast.success(`${stateDefinitions[kind].label} created`, {
        description: `${created.name} was selected.`
      });
      return { label: created.name, value: String(created.id) };
    } catch (error) {
      toast.error(`Unable to create ${stateDefinitions[kind].label.toLowerCase()}`, {
        description: error instanceof Error ? error.message : "Please try again."
      });
      return undefined;
    }
...
function toFormValue(kind: StateKind, record: StateRecord | null): StateSavePayload {
  if (record) {
    const { id: _id, ...value } = record;
    return value;
  }
  return emptyStatePayload(kind);
}

function emptyStatePayload(kind?: StateKind): StateSavePayload {
  return {
    areaName: null,
    capital: null,
    cityId: null,
...
function inlineCreateStatePayload(): StateSavePayload {
  return {
    ...emptyStatePayload(),
    capital: "-",
    currencyCode: "-",
...
export function StateWorkspace() {
  return <StateModuleShell kind="state" />;
}
