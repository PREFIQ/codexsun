...
import { districtDefinitions } from "./district.types";
import {
  createDistrictRecord,
  forceDeleteDistrictRecord,
  listDistrictRecords,
  suspendDistrictRecord,
...
export function DistrictModuleShell({ kind }: { kind: DistrictKind }) {
  const definition = districtDefinitions[kind];
  ...
  const recordsQuery = useQuery({
    ...
    queryFn: () => listDistrictRecords(definition.path)
  });
  const parentQueries = useDistrictParents(kind);
  const mutation = useMutation({
    mutationFn: (payload: DistrictSavePayload) =>
      editing
        ? updateDistrictRecord(definition.path, editing.id, payload)
        : createDistrictRecord(definition.path, payload),
  ...
  const rowActionMutation = useMutation({
    mutationFn: async ({ record, type }: { record: DistrictRecord; type: "delete" | "suspend" }) =>
      type === "delete"
        ? forceDeleteDistrictRecord(definition.path, record.id)
        : suspendDistrictRecord(definition.path, record.id),
...
export function DistrictTable({
  ...
  onForceDelete?: (record: DistrictRecord) => void;
  onSuspend?: (record: DistrictRecord) => void;
  onView?: (record: DistrictRecord) => void;
        ...
        <table className="w-full min-w-[760px] border-collapse text-sm">
          ...
          <tbody>
            {records.map((record, index) => {
              const canMutate = canMutateDistrictRow(definition.kind, record);
              const protectedRow = isProtectedDistrictRow(definition.kind, record);
              return (
                  ...
                  {definition.columns.map((column) => (
                    ...
                    >
                      {column.key === "status" ? (
                        ...
                        />
                      ) : isPrimaryDistrictColumn(definition.kind, column.key) && canMutate ? (
                        <button
...
function canMutateDistrictRow(kind: DistrictKind, record: DistrictRecord) {
  if (isProtectedDistrictRow(kind, record)) return false;
  return true;
}

function isProtectedDistrictRow(kind: DistrictKind, record: DistrictRecord) {
...
function isPrimaryDistrictColumn(kind: DistrictKind, key: keyof DistrictRecord) {
  return key === "name" || (kind === "pincode" && key === "pincode");
}

type DistrictParents = {
  cities: DistrictRecord[];
  countries: DistrictRecord[];
  districts: DistrictRecord[];
  states: DistrictRecord[];
};

function useDistrictParents(kind: DistrictKind): DistrictParents {
  const countries = useQuery({
    enabled: kind === "state" || kind === "district" || kind === "city" || kind === "pincode",
    queryKey: ["core", "location", "country"],
    queryFn: () => listDistrictRecords(districtDefinitions.country.path)
  });
  const states = useQuery({
    enabled: kind === "district" || kind === "city" || kind === "pincode",
    queryKey: ["core", "location", "state"],
    queryFn: () => listDistrictRecords(districtDefinitions.state.path)
  });
  const districts = useQuery({
    enabled: kind === "city" || kind === "pincode",
    queryKey: ["core", "location", "district"],
    queryFn: () => listDistrictRecords(districtDefinitions.district.path)
  });
  const cities = useQuery({
    enabled: kind === "pincode",
    queryKey: ["core", "location", "city"],
    queryFn: () => listDistrictRecords(districtDefinitions.city.path)
  });
  return {
    cities: cities.data ?? [],
...
export function DistrictUpsertDialog({
  definition,
  error,
  loading,
  onClose,
  ...
  loading: boolean;
  onClose: () => void;
  onSubmit: (payload: DistrictSavePayload) => void;
  open: boolean;
  parents: DistrictParents;
...
function DistrictForm({
  ...
  definition: DistrictDefinition;
  error: string;
  initialValue: DistrictSavePayload;
  loading: boolean;
  onCancel: () => void;
  onSubmit: (payload: DistrictSavePayload) => void;
  parents: DistrictParents;
  ...
  const update = (patch: Partial<DistrictSavePayload>) => {
  ...
  const createParent = async (
    ...
    const payload = inlineCreateDistrictPayload();
    ...
    try {
      const created = await createDistrictRecord(districtDefinitions[kind].path, payload);
      await queryClient.invalidateQueries({ queryKey: ["core", "location", kind] });
      toast.success(`${districtDefinitions[kind].label} created`, {
        description: `${created.name} was selected.`
      });
      return { label: created.name, value: String(created.id) };
    } catch (error) {
      toast.error(`Unable to create ${districtDefinitions[kind].label.toLowerCase()}`, {
        description: error instanceof Error ? error.message : "Please try again."
      });
      return undefined;
    }
...
function toFormValue(kind: DistrictKind, record: DistrictRecord | null): DistrictSavePayload {
  if (record) {
    const { id: _id, ...value } = record;
    return value;
  }
  return emptyDistrictPayload(kind);
}

function emptyDistrictPayload(kind?: DistrictKind): DistrictSavePayload {
  return {
    areaName: null,
    capital: null,
    cityId: null,
...
function inlineCreateDistrictPayload(): DistrictSavePayload {
  return {
    ...emptyDistrictPayload(),
    capital: "-",
    currencyCode: "-",
    dialCode: "-",
    gstStateCode: "-",
...
export function DistrictWorkspace() {
  return <DistrictModuleShell kind="district" />;
}
