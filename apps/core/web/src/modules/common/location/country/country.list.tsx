...
import { countryDefinitions } from "./country.types";
import {
  createCountryRecord,
  forceDeleteCountryRecord,
  listCountryRecords,
  suspendCountryRecord,
  updateCountryRecord
...
export function CountryModuleShell({ kind }: { kind: CountryKind }) {
  const definition = countryDefinitions[kind];
  ...
  const recordsQuery = useQuery({
    ...
    queryFn: () => listCountryRecords(definition.path)
  });
  const parentQueries = useCountryParents(kind);
  const mutation = useMutation({
    mutationFn: (payload: CountrySavePayload) =>
      editing
        ? updateCountryRecord(definition.path, editing.id, payload)
        : createCountryRecord(definition.path, payload),
  ...
  const rowActionMutation = useMutation({
    mutationFn: async ({ record, type }: { record: CountryRecord; type: "delete" | "suspend" }) =>
      type === "delete"
        ? forceDeleteCountryRecord(definition.path, record.id)
        : suspendCountryRecord(definition.path, record.id),
        ...
        <table className="w-full min-w-[760px] border-collapse text-sm">
          ...
          <tbody>
            {records.map((record, index) => {
              const canMutate = canMutateCountryRow(definition.kind, record);
              const protectedRow = isProtectedCountryRow(definition.kind, record);
              return (
                  ...
                  {definition.columns.map((column) => (
                    ...
                    >
                      {column.key === "status" ? (
                        ...
                        />
                      ) : isPrimaryCountryColumn(definition.kind, column.key) && canMutate ? (
                        <button
...
function canMutateCountryRow(kind: CountryKind, record: CountryRecord) {
  if (isProtectedCountryRow(kind, record)) return false;
  return true;
}

function isProtectedCountryRow(kind: CountryKind, record: CountryRecord) {
...
function isPrimaryCountryColumn(kind: CountryKind, key: keyof CountryRecord) {
  return key === "name" || (kind === "pincode" && key === "pincode");
}

type CountryParents = {
  cities: CountryRecord[];
  countries: CountryRecord[];
  districts: CountryRecord[];
  states: CountryRecord[];
};

function useCountryParents(kind: CountryKind): CountryParents {
  const countries = useQuery({
    enabled: kind === "state" || kind === "district" || kind === "city" || kind === "pincode",
    queryKey: ["core", "location", "country"],
    queryFn: () => listCountryRecords(countryDefinitions.country.path)
  });
  const states = useQuery({
    enabled: kind === "district" || kind === "city" || kind === "pincode",
    queryKey: ["core", "location", "state"],
    queryFn: () => listCountryRecords(countryDefinitions.state.path)
  });
  const districts = useQuery({
    enabled: kind === "city" || kind === "pincode",
    queryKey: ["core", "location", "district"],
    queryFn: () => listCountryRecords(countryDefinitions.district.path)
  });
  const cities = useQuery({
    enabled: kind === "pincode",
    queryKey: ["core", "location", "city"],
    queryFn: () => listCountryRecords(countryDefinitions.city.path)
  });
  return {
    cities: cities.data ?? [],
...
export function CountryUpsertDialog({
  definition,
  error,
  loading,
  onClose,
...
function CountryForm({
  ...
  const update = (patch: Partial<CountrySavePayload>) => {
  ...
  const createParent = async (
    ...
    const payload = inlineCreateCountryPayload();
    ...
    try {
      const created = await createCountryRecord(countryDefinitions[kind].path, payload);
      await queryClient.invalidateQueries({ queryKey: ["core", "location", kind] });
      toast.success(`${countryDefinitions[kind].label} created`, {
        description: `${created.name} was selected.`
      });
      return { label: created.name, value: String(created.id) };
    } catch (error) {
      toast.error(`Unable to create ${countryDefinitions[kind].label.toLowerCase()}`, {
        description: error instanceof Error ? error.message : "Please try again."
      });
      return undefined;
    }
...
function toFormValue(kind: CountryKind, record: CountryRecord | null): CountrySavePayload {
  if (record) {
    const { id: _id, ...value } = record;
    return value;
  }
  return emptyCountryPayload(kind);
}

function emptyCountryPayload(kind?: CountryKind): CountrySavePayload {
  return {
    areaName: null,
    capital: null,
    cityId: null,
...
function inlineCreateCountryPayload(): CountrySavePayload {
  return {
    ...emptyCountryPayload(),
    capital: "-",
    currencyCode: "-",
...
export function CountryWorkspace() {
  return <CountryModuleShell kind="country" />;
}
