export type CityKind = "country" | "state" | "district" | "city" | "pincode";
...
export type CitySavePayload = Omit<CityRecord, "id">;

export type CityDefinition = {
  columns: Array<{ key: keyof CityRecord; label: string }>;
  kind: CityKind;
  label: string;
  path: string;
  plural: string;
...
export const cityDefinitions: Record<CityKind, CityDefinition> = {
  country: {
    columns: [
