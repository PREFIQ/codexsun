import { destinationsDefinition } from "./destinations.types";
...
export function DestinationsShell({ definition }: { definition: DestinationsDefinition }) {
  ...
  const [deleting, setDeleting] = useState<DestinationsRecord | null>(null);
  const query = useQuery({
    queryFn: () => listDestinations(definition.path),
    queryKey: ["core", "common", definition.key]
  });
  const save = useMutation({
    mutationFn: (payload: Record<string, DestinationsValue>) =>
      editing
        ? updateDestinations(definition.path, editing.id, payload)
        : createDestinations(definition.path, payload),
    onSuccess: async () => {
  ...
  const toggle = useMutation({
    mutationFn: (record: DestinationsRecord) =>
      setDestinationsActive(definition.path, record.id, !record.isActive),
    onSuccess: async () =>
      queryClient.invalidateQueries({ queryKey: ["core", "common", definition.key] })
  });
  const forceDelete = useMutation({
    mutationFn: (record: DestinationsRecord) => forceDeleteDestinations(definition.path, record.id),
...
export function DestinationsForm({
  ...
  loading: boolean;
  onClose: () => void;
  onSubmit: (payload: Record<string, DestinationsValue>) => void;
  open: boolean;
  ...
  const [value, setValue] = useState<Record<string, DestinationsValue>>({
    ...initial,
...
function formatValue(value: DestinationsValue, type: string) {
...
function defaultValue(type: string): DestinationsValue {
...
function isFinalizedContactMaster(definition: DestinationsDefinition) {
  return FINALIZED_COMMON_MASTER_KEYS.has(definition.key);
}
function isPlaceholder(record: DestinationsRecord, definition: DestinationsDefinition) {
...
export function DestinationsWorkspace() {
  return <DestinationsShell definition={destinationsDefinition} />;
}
