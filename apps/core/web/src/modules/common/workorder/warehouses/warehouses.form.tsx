import { warehousesDefinition } from "./warehouses.types";
...
import type { WarehousesDefinition, WarehousesRecord, WarehousesValue } from "./warehouses.types";

export function WarehousesShell({ definition }: { definition: WarehousesDefinition }) {
  ...
  const [deleting, setDeleting] = useState<WarehousesRecord | null>(null);
  const query = useQuery({
    queryFn: () => listWarehouses(definition.path),
    queryKey: ["core", "common", definition.key]
  });
  const save = useMutation({
    mutationFn: (payload: Record<string, WarehousesValue>) =>
      editing
        ? updateWarehouses(definition.path, editing.id, payload)
        : createWarehouses(definition.path, payload),
    onSuccess: async () => {
  ...
  const toggle = useMutation({
    mutationFn: (record: WarehousesRecord) =>
      setWarehousesActive(definition.path, record.id, !record.isActive),
    onSuccess: async () =>
      queryClient.invalidateQueries({ queryKey: ["core", "common", definition.key] })
  });
  const forceDelete = useMutation({
    mutationFn: (record: WarehousesRecord) => forceDeleteWarehouses(definition.path, record.id),
...
export function WarehousesForm({
  ...
  loading: boolean;
  onClose: () => void;
  onSubmit: (payload: Record<string, WarehousesValue>) => void;
  open: boolean;
  ...
  const [value, setValue] = useState<Record<string, WarehousesValue>>({
    ...initial,
...
function formatValue(value: WarehousesValue, type: string) {
...
function defaultValue(type: string): WarehousesValue {
...
function isFinalizedContactMaster(definition: WarehousesDefinition) {
  return FINALIZED_COMMON_MASTER_KEYS.has(definition.key);
}
function isPlaceholder(record: WarehousesRecord, definition: WarehousesDefinition) {
...
export function WarehousesWorkspace() {
  return <WarehousesShell definition={warehousesDefinition} />;
}
