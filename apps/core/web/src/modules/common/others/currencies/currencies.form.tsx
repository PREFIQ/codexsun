import { currenciesDefinition } from "./currencies.types";
...
import type { CurrenciesDefinition, CurrenciesRecord, CurrenciesValue } from "./currencies.types";

export function CurrenciesShell({ definition }: { definition: CurrenciesDefinition }) {
  ...
  const [deleting, setDeleting] = useState<CurrenciesRecord | null>(null);
  const query = useQuery({
    queryFn: () => listCurrencies(definition.path),
    queryKey: ["core", "common", definition.key]
  });
  const save = useMutation({
    mutationFn: (payload: Record<string, CurrenciesValue>) =>
      editing
        ? updateCurrencies(definition.path, editing.id, payload)
        : createCurrencies(definition.path, payload),
    onSuccess: async () => {
  ...
  const toggle = useMutation({
    mutationFn: (record: CurrenciesRecord) =>
      setCurrenciesActive(definition.path, record.id, !record.isActive),
    onSuccess: async () =>
      queryClient.invalidateQueries({ queryKey: ["core", "common", definition.key] })
  });
  const forceDelete = useMutation({
    mutationFn: (record: CurrenciesRecord) => forceDeleteCurrencies(definition.path, record.id),
...
export function CurrenciesForm({
  ...
  loading: boolean;
  onClose: () => void;
  onSubmit: (payload: Record<string, CurrenciesValue>) => void;
  open: boolean;
  ...
  const [value, setValue] = useState<Record<string, CurrenciesValue>>({
    ...initial,
...
function formatValue(value: CurrenciesValue, type: string) {
...
function defaultValue(type: string): CurrenciesValue {
...
function isFinalizedContactMaster(definition: CurrenciesDefinition) {
  return FINALIZED_COMMON_MASTER_KEYS.has(definition.key);
}
function isPlaceholder(record: CurrenciesRecord, definition: CurrenciesDefinition) {
...
export function CurrenciesWorkspace() {
  return <CurrenciesShell definition={currenciesDefinition} />;
}
