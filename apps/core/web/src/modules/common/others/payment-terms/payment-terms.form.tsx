import { paymentTermsDefinition } from "./payment-terms.types";
...
export function PaymentTermsShell({ definition }: { definition: PaymentTermsDefinition }) {
  ...
  const [deleting, setDeleting] = useState<PaymentTermsRecord | null>(null);
  const query = useQuery({
    queryFn: () => listPaymentTerms(definition.path),
    queryKey: ["core", "common", definition.key]
  });
  const save = useMutation({
    mutationFn: (payload: Record<string, PaymentTermsValue>) =>
      editing
        ? updatePaymentTerms(definition.path, editing.id, payload)
        : createPaymentTerms(definition.path, payload),
    onSuccess: async () => {
  ...
  const toggle = useMutation({
    mutationFn: (record: PaymentTermsRecord) =>
      setPaymentTermsActive(definition.path, record.id, !record.isActive),
    onSuccess: async () =>
      queryClient.invalidateQueries({ queryKey: ["core", "common", definition.key] })
  });
  const forceDelete = useMutation({
    mutationFn: (record: PaymentTermsRecord) => forceDeletePaymentTerms(definition.path, record.id),
...
export function PaymentTermsForm({
  ...
  loading: boolean;
  onClose: () => void;
  onSubmit: (payload: Record<string, PaymentTermsValue>) => void;
  open: boolean;
  ...
  const [value, setValue] = useState<Record<string, PaymentTermsValue>>({
    ...initial,
...
function formatValue(value: PaymentTermsValue, type: string) {
...
function defaultValue(type: string): PaymentTermsValue {
...
function isFinalizedContactMaster(definition: PaymentTermsDefinition) {
  return FINALIZED_COMMON_MASTER_KEYS.has(definition.key);
}
function isPlaceholder(record: PaymentTermsRecord, definition: PaymentTermsDefinition) {
...
export function PaymentTermsWorkspace() {
  return <PaymentTermsShell definition={paymentTermsDefinition} />;
}
