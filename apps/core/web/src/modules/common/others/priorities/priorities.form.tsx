import { prioritiesDefinition } from "./priorities.types";
...
import type { PrioritiesDefinition, PrioritiesRecord, PrioritiesValue } from "./priorities.types";

export function PrioritiesShell({ definition }: { definition: PrioritiesDefinition }) {
  ...
  const [deleting, setDeleting] = useState<PrioritiesRecord | null>(null);
  const query = useQuery({
    queryFn: () => listPriorities(definition.path),
    queryKey: ["core", "common", definition.key]
  });
  const save = useMutation({
    mutationFn: (payload: Record<string, PrioritiesValue>) =>
      editing
        ? updatePriorities(definition.path, editing.id, payload)
        : createPriorities(definition.path, payload),
    onSuccess: async () => {
  ...
  const toggle = useMutation({
    mutationFn: (record: PrioritiesRecord) =>
      setPrioritiesActive(definition.path, record.id, !record.isActive),
    onSuccess: async () =>
      queryClient.invalidateQueries({ queryKey: ["core", "common", definition.key] })
  });
  const forceDelete = useMutation({
    mutationFn: (record: PrioritiesRecord) => forceDeletePriorities(definition.path, record.id),
...
export function PrioritiesForm({
  ...
  loading: boolean;
  onClose: () => void;
  onSubmit: (payload: Record<string, PrioritiesValue>) => void;
  open: boolean;
  ...
  const [value, setValue] = useState<Record<string, PrioritiesValue>>({
    ...initial,
...
function formatValue(value: PrioritiesValue, type: string) {
...
function defaultValue(type: string): PrioritiesValue {
...
function isFinalizedContactMaster(definition: PrioritiesDefinition) {
  return FINALIZED_COMMON_MASTER_KEYS.has(definition.key);
}
function isPlaceholder(record: PrioritiesRecord, definition: PrioritiesDefinition) {
...
export function PrioritiesWorkspace() {
  return <PrioritiesShell definition={prioritiesDefinition} />;
}
