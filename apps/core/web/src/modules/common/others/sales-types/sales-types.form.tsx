import { salesTypesDefinition } from "./sales-types.types";
...
import type { SalesTypesDefinition, SalesTypesRecord, SalesTypesValue } from "./sales-types.types";

export function SalesTypesShell({ definition }: { definition: SalesTypesDefinition }) {
  ...
  const [deleting, setDeleting] = useState<SalesTypesRecord | null>(null);
  const query = useQuery({
    queryFn: () => listSalesTypes(definition.path),
    queryKey: ["core", "common", definition.key]
  });
  const save = useMutation({
    mutationFn: (payload: Record<string, SalesTypesValue>) =>
      editing
        ? updateSalesTypes(definition.path, editing.id, payload)
        : createSalesTypes(definition.path, payload),
    onSuccess: async () => {
  ...
  const toggle = useMutation({
    mutationFn: (record: SalesTypesRecord) =>
      setSalesTypesActive(definition.path, record.id, !record.isActive),
    onSuccess: async () =>
      queryClient.invalidateQueries({ queryKey: ["core", "common", definition.key] })
  });
  const forceDelete = useMutation({
    mutationFn: (record: SalesTypesRecord) => forceDeleteSalesTypes(definition.path, record.id),
...
export function SalesTypesForm({
  ...
  loading: boolean;
  onClose: () => void;
  onSubmit: (payload: Record<string, SalesTypesValue>) => void;
  open: boolean;
  ...
  const [value, setValue] = useState<Record<string, SalesTypesValue>>({
    ...initial,
...
function formatValue(value: SalesTypesValue, type: string) {
...
function defaultValue(type: string): SalesTypesValue {
...
function isFinalizedContactMaster(definition: SalesTypesDefinition) {
  return FINALIZED_COMMON_MASTER_KEYS.has(definition.key);
}
function isPlaceholder(record: SalesTypesRecord, definition: SalesTypesDefinition) {
...
export function SalesTypesWorkspace() {
  return <SalesTypesShell definition={salesTypesDefinition} />;
}
