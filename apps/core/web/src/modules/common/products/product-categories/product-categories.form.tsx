import { productCategoriesDefinition } from "./product-categories.types";
...
export function ProductCategoriesShell({
  ...
  definition: ProductCategoriesDefinition;
}) {
  ...
  const [deleting, setDeleting] = useState<ProductCategoriesRecord | null>(null);
  const query = useQuery({
    queryFn: () => listProductCategories(definition.path),
    queryKey: ["core", "common", definition.key]
  });
  const save = useMutation({
    mutationFn: (payload: Record<string, ProductCategoriesValue>) =>
      editing
        ? updateProductCategories(definition.path, editing.id, payload)
        : createProductCategories(definition.path, payload),
    onSuccess: async () => {
  ...
  const toggle = useMutation({
    mutationFn: (record: ProductCategoriesRecord) =>
      setProductCategoriesActive(definition.path, record.id, !record.isActive),
    onSuccess: async () =>
      queryClient.invalidateQueries({ queryKey: ["core", "common", definition.key] })
  });
  const forceDelete = useMutation({
    mutationFn: (record: ProductCategoriesRecord) =>
      forceDeleteProductCategories(definition.path, record.id),
...
export function ProductCategoriesList({
  ...
  onToggle: (record: ProductCategoriesRecord) => void;
...
export function ProductCategoriesForm({
  ...
  loading: boolean;
  onClose: () => void;
  onSubmit: (payload: Record<string, ProductCategoriesValue>) => void;
  open: boolean;
  ...
  const [value, setValue] = useState<Record<string, ProductCategoriesValue>>({
    ...initial,
...
function formatValue(value: ProductCategoriesValue, type: string) {
...
function defaultValue(type: string): ProductCategoriesValue {
...
function isFinalizedContactMaster(definition: ProductCategoriesDefinition) {
  return FINALIZED_COMMON_MASTER_KEYS.has(definition.key);
}
function isPlaceholder(record: ProductCategoriesRecord, definition: ProductCategoriesDefinition) {
...
export function ProductCategoriesWorkspace() {
  return <ProductCategoriesShell definition={productCategoriesDefinition} />;
}
