import { productGroupsDefinition } from "./product-groups.types";
...
export function ProductGroupsShell({ definition }: { definition: ProductGroupsDefinition }) {
  ...
  const [deleting, setDeleting] = useState<ProductGroupsRecord | null>(null);
  const query = useQuery({
    queryFn: () => listProductGroups(definition.path),
    queryKey: ["core", "common", definition.key]
  });
  const save = useMutation({
    mutationFn: (payload: Record<string, ProductGroupsValue>) =>
      editing
        ? updateProductGroups(definition.path, editing.id, payload)
        : createProductGroups(definition.path, payload),
    onSuccess: async () => {
  ...
  const toggle = useMutation({
    mutationFn: (record: ProductGroupsRecord) =>
      setProductGroupsActive(definition.path, record.id, !record.isActive),
    onSuccess: async () =>
      queryClient.invalidateQueries({ queryKey: ["core", "common", definition.key] })
  });
  const forceDelete = useMutation({
    mutationFn: (record: ProductGroupsRecord) =>
      forceDeleteProductGroups(definition.path, record.id),
...
export function ProductGroupsForm({
  ...
  loading: boolean;
  onClose: () => void;
  onSubmit: (payload: Record<string, ProductGroupsValue>) => void;
  open: boolean;
  ...
  const [value, setValue] = useState<Record<string, ProductGroupsValue>>({
    ...initial,
...
function formatValue(value: ProductGroupsValue, type: string) {
...
function defaultValue(type: string): ProductGroupsValue {
...
function isFinalizedContactMaster(definition: ProductGroupsDefinition) {
  return FINALIZED_COMMON_MASTER_KEYS.has(definition.key);
}
function isPlaceholder(record: ProductGroupsRecord, definition: ProductGroupsDefinition) {
...
export function ProductGroupsWorkspace() {
  return <ProductGroupsShell definition={productGroupsDefinition} />;
}
