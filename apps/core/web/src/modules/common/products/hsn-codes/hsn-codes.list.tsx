import { hsnCodesDefinition } from "./hsn-codes.types";
...
import type { HsnCodesDefinition, HsnCodesRecord, HsnCodesValue } from "./hsn-codes.types";

export function HsnCodesShell({ definition }: { definition: HsnCodesDefinition }) {
  ...
  const [deleting, setDeleting] = useState<HsnCodesRecord | null>(null);
  const query = useQuery({
    queryFn: () => listHsnCodes(definition.path),
    queryKey: ["core", "common", definition.key]
  });
  const save = useMutation({
    mutationFn: (payload: Record<string, HsnCodesValue>) =>
      editing
        ? updateHsnCodes(definition.path, editing.id, payload)
        : createHsnCodes(definition.path, payload),
    onSuccess: async () => {
  ...
  const toggle = useMutation({
    mutationFn: (record: HsnCodesRecord) =>
      setHsnCodesActive(definition.path, record.id, !record.isActive),
    onSuccess: async () =>
      queryClient.invalidateQueries({ queryKey: ["core", "common", definition.key] })
  });
  const forceDelete = useMutation({
    mutationFn: (record: HsnCodesRecord) => forceDeleteHsnCodes(definition.path, record.id),
...
export function HsnCodesForm({
  ...
  loading: boolean;
  onClose: () => void;
  onSubmit: (payload: Record<string, HsnCodesValue>) => void;
  open: boolean;
  ...
  const [value, setValue] = useState<Record<string, HsnCodesValue>>({
    ...initial,
...
function formatValue(value: HsnCodesValue, type: string) {
...
function defaultValue(type: string): HsnCodesValue {
...
function isFinalizedContactMaster(definition: HsnCodesDefinition) {
  return FINALIZED_COMMON_MASTER_KEYS.has(definition.key);
}
function isPlaceholder(record: HsnCodesRecord, definition: HsnCodesDefinition) {
...
export function HsnCodesWorkspace() {
  return <HsnCodesShell definition={hsnCodesDefinition} />;
}
