import { brandsDefinition } from "./brands.types";
...
import type { BrandsDefinition, BrandsRecord, BrandsValue } from "./brands.types";

export function BrandsShell({ definition }: { definition: BrandsDefinition }) {
  ...
  const [deleting, setDeleting] = useState<BrandsRecord | null>(null);
  const query = useQuery({
    queryFn: () => listBrands(definition.path),
    queryKey: ["core", "common", definition.key]
  });
  const save = useMutation({
    mutationFn: (payload: Record<string, BrandsValue>) =>
      editing
        ? updateBrands(definition.path, editing.id, payload)
        : createBrands(definition.path, payload),
    onSuccess: async () => {
  ...
  const toggle = useMutation({
    mutationFn: (record: BrandsRecord) =>
      setBrandsActive(definition.path, record.id, !record.isActive),
    onSuccess: async () =>
      queryClient.invalidateQueries({ queryKey: ["core", "common", definition.key] })
  });
  const forceDelete = useMutation({
    mutationFn: (record: BrandsRecord) => forceDeleteBrands(definition.path, record.id),
...
export function BrandsForm({
  ...
  loading: boolean;
  onClose: () => void;
  onSubmit: (payload: Record<string, BrandsValue>) => void;
  open: boolean;
  ...
  const [value, setValue] = useState<Record<string, BrandsValue>>({
    ...initial,
...
function formatValue(value: BrandsValue, type: string) {
...
function defaultValue(type: string): BrandsValue {
...
function isFinalizedContactMaster(definition: BrandsDefinition) {
  return FINALIZED_COMMON_MASTER_KEYS.has(definition.key);
}
function isPlaceholder(record: BrandsRecord, definition: BrandsDefinition) {
...
export function BrandsWorkspace() {
  return <BrandsShell definition={brandsDefinition} />;
}
