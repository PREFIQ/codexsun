import { productTypesDefinition } from "./product-types.types";
...
export function ProductTypesShell({ definition }: { definition: ProductTypesDefinition }) {
  ...
  const [deleting, setDeleting] = useState<ProductTypesRecord | null>(null);
  const query = useQuery({
    queryFn: () => listProductTypes(definition.path),
    queryKey: ["core", "common", definition.key]
  });
  const save = useMutation({
    mutationFn: (payload: Record<string, ProductTypesValue>) =>
      editing
        ? updateProductTypes(definition.path, editing.id, payload)
        : createProductTypes(definition.path, payload),
    onSuccess: async () => {
  ...
  const toggle = useMutation({
    mutationFn: (record: ProductTypesRecord) =>
      setProductTypesActive(definition.path, record.id, !record.isActive),
    onSuccess: async () =>
      queryClient.invalidateQueries({ queryKey: ["core", "common", definition.key] })
  });
  const forceDelete = useMutation({
    mutationFn: (record: ProductTypesRecord) => forceDeleteProductTypes(definition.path, record.id),
...
export function ProductTypesForm({
  ...
  loading: boolean;
  onClose: () => void;
  onSubmit: (payload: Record<string, ProductTypesValue>) => void;
  open: boolean;
  ...
  const [value, setValue] = useState<Record<string, ProductTypesValue>>({
    ...initial,
...
function formatValue(value: ProductTypesValue, type: string) {
...
function defaultValue(type: string): ProductTypesValue {
...
function isFinalizedContactMaster(definition: ProductTypesDefinition) {
  return FINALIZED_COMMON_MASTER_KEYS.has(definition.key);
}
function isPlaceholder(record: ProductTypesRecord, definition: ProductTypesDefinition) {
...
export function ProductTypesWorkspace() {
  return <ProductTypesShell definition={productTypesDefinition} />;
}
