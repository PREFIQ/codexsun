import { unitsDefinition } from "./units.types";
...
import type { UnitsDefinition, UnitsRecord, UnitsValue } from "./units.types";

export function UnitsShell({ definition }: { definition: UnitsDefinition }) {
  ...
  const [deleting, setDeleting] = useState<UnitsRecord | null>(null);
  const query = useQuery({
    queryFn: () => listUnits(definition.path),
    queryKey: ["core", "common", definition.key]
  });
  const save = useMutation({
    mutationFn: (payload: Record<string, UnitsValue>) =>
      editing
        ? updateUnits(definition.path, editing.id, payload)
        : createUnits(definition.path, payload),
    onSuccess: async () => {
  ...
  const toggle = useMutation({
    mutationFn: (record: UnitsRecord) =>
      setUnitsActive(definition.path, record.id, !record.isActive),
    onSuccess: async () =>
      queryClient.invalidateQueries({ queryKey: ["core", "common", definition.key] })
  });
  const forceDelete = useMutation({
    mutationFn: (record: UnitsRecord) => forceDeleteUnits(definition.path, record.id),
...
export function UnitsList({
  ...
  onToggle: (record: UnitsRecord) => void;
  records: UnitsRecord[];
  startIndex?: number;
}) {
  return (
...
export function UnitsForm({
  ...
  loading: boolean;
  onClose: () => void;
  onSubmit: (payload: Record<string, UnitsValue>) => void;
  open: boolean;
  ...
  const [value, setValue] = useState<Record<string, UnitsValue>>({
    ...initial,
...
function formatValue(value: UnitsValue, type: string) {
...
function defaultValue(type: string): UnitsValue {
...
function isFinalizedContactMaster(definition: UnitsDefinition) {
  return FINALIZED_COMMON_MASTER_KEYS.has(definition.key);
}
function isPlaceholder(record: UnitsRecord, definition: UnitsDefinition) {
...
export function UnitsWorkspace() {
  return <UnitsShell definition={unitsDefinition} />;
}
