import { taxesDefinition } from "./taxes.types";
...
import type { TaxesDefinition, TaxesRecord, TaxesValue } from "./taxes.types";

export function TaxesShell({ definition }: { definition: TaxesDefinition }) {
  ...
  const [deleting, setDeleting] = useState<TaxesRecord | null>(null);
  const query = useQuery({
    queryFn: () => listTaxes(definition.path),
    queryKey: ["core", "common", definition.key]
  });
  const save = useMutation({
    mutationFn: (payload: Record<string, TaxesValue>) =>
      editing
        ? updateTaxes(definition.path, editing.id, payload)
        : createTaxes(definition.path, payload),
    onSuccess: async () => {
  ...
  const toggle = useMutation({
    mutationFn: (record: TaxesRecord) =>
      setTaxesActive(definition.path, record.id, !record.isActive),
    onSuccess: async () =>
      queryClient.invalidateQueries({ queryKey: ["core", "common", definition.key] })
  });
  const forceDelete = useMutation({
    mutationFn: (record: TaxesRecord) => forceDeleteTaxes(definition.path, record.id),
...
export function TaxesForm({
  ...
  loading: boolean;
  onClose: () => void;
  onSubmit: (payload: Record<string, TaxesValue>) => void;
  open: boolean;
  ...
  const [value, setValue] = useState<Record<string, TaxesValue>>({
    ...initial,
...
function formatValue(value: TaxesValue, type: string) {
...
function defaultValue(type: string): TaxesValue {
...
function isFinalizedContactMaster(definition: TaxesDefinition) {
  return FINALIZED_COMMON_MASTER_KEYS.has(definition.key);
}
function isPlaceholder(record: TaxesRecord, definition: TaxesDefinition) {
...
export function TaxesWorkspace() {
  return <TaxesShell definition={taxesDefinition} />;
}
