import { stylesDefinition } from "./styles.types";
...
import type { StylesDefinition, StylesRecord, StylesValue } from "./styles.types";

export function StylesShell({ definition }: { definition: StylesDefinition }) {
  ...
  const [deleting, setDeleting] = useState<StylesRecord | null>(null);
  const query = useQuery({
    queryFn: () => listStyles(definition.path),
    queryKey: ["core", "common", definition.key]
  });
  const save = useMutation({
    mutationFn: (payload: Record<string, StylesValue>) =>
      editing
        ? updateStyles(definition.path, editing.id, payload)
        : createStyles(definition.path, payload),
    onSuccess: async () => {
  ...
  const toggle = useMutation({
    mutationFn: (record: StylesRecord) =>
      setStylesActive(definition.path, record.id, !record.isActive),
    onSuccess: async () =>
      queryClient.invalidateQueries({ queryKey: ["core", "common", definition.key] })
  });
  const forceDelete = useMutation({
    mutationFn: (record: StylesRecord) => forceDeleteStyles(definition.path, record.id),
...
export function StylesForm({
  ...
  loading: boolean;
  onClose: () => void;
  onSubmit: (payload: Record<string, StylesValue>) => void;
  open: boolean;
  ...
  const [value, setValue] = useState<Record<string, StylesValue>>({
    ...initial,
...
function formatValue(value: StylesValue, type: string) {
...
function defaultValue(type: string): StylesValue {
...
function isFinalizedContactMaster(definition: StylesDefinition) {
  return FINALIZED_COMMON_MASTER_KEYS.has(definition.key);
}
function isPlaceholder(record: StylesRecord, definition: StylesDefinition) {
...
export function StylesWorkspace() {
  return <StylesShell definition={stylesDefinition} />;
}
