import { sizesDefinition } from "./sizes.types";
...
import type { SizesDefinition, SizesRecord, SizesValue } from "./sizes.types";

export function SizesShell({ definition }: { definition: SizesDefinition }) {
  ...
  const [deleting, setDeleting] = useState<SizesRecord | null>(null);
  const query = useQuery({
    queryFn: () => listSizes(definition.path),
    queryKey: ["core", "common", definition.key]
  });
  const save = useMutation({
    mutationFn: (payload: Record<string, SizesValue>) =>
      editing
        ? updateSizes(definition.path, editing.id, payload)
        : createSizes(definition.path, payload),
    onSuccess: async () => {
  ...
  const toggle = useMutation({
    mutationFn: (record: SizesRecord) =>
      setSizesActive(definition.path, record.id, !record.isActive),
    onSuccess: async () =>
      queryClient.invalidateQueries({ queryKey: ["core", "common", definition.key] })
  });
  const forceDelete = useMutation({
    mutationFn: (record: SizesRecord) => forceDeleteSizes(definition.path, record.id),
...
export function SizesForm({
  ...
  loading: boolean;
  onClose: () => void;
  onSubmit: (payload: Record<string, SizesValue>) => void;
  open: boolean;
  ...
  const [value, setValue] = useState<Record<string, SizesValue>>({
    ...initial,
...
function formatValue(value: SizesValue, type: string) {
...
function defaultValue(type: string): SizesValue {
...
function isFinalizedContactMaster(definition: SizesDefinition) {
  return FINALIZED_COMMON_MASTER_KEYS.has(definition.key);
}
function isPlaceholder(record: SizesRecord, definition: SizesDefinition) {
...
export function SizesWorkspace() {
  return <SizesShell definition={sizesDefinition} />;
}
