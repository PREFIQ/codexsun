import { coloursDefinition } from "./colours.types";
...
import type { ColoursDefinition, ColoursRecord, ColoursValue } from "./colours.types";

export function ColoursShell({ definition }: { definition: ColoursDefinition }) {
  ...
  const [deleting, setDeleting] = useState<ColoursRecord | null>(null);
  const query = useQuery({
    queryFn: () => listColours(definition.path),
    queryKey: ["core", "common", definition.key]
  });
  const save = useMutation({
    mutationFn: (payload: Record<string, ColoursValue>) =>
      editing
        ? updateColours(definition.path, editing.id, payload)
        : createColours(definition.path, payload),
    onSuccess: async () => {
  ...
  const toggle = useMutation({
    mutationFn: (record: ColoursRecord) =>
      setColoursActive(definition.path, record.id, !record.isActive),
    onSuccess: async () =>
      queryClient.invalidateQueries({ queryKey: ["core", "common", definition.key] })
  });
  const forceDelete = useMutation({
    mutationFn: (record: ColoursRecord) => forceDeleteColours(definition.path, record.id),
...
export function ColoursForm({
  ...
  loading: boolean;
  onClose: () => void;
  onSubmit: (payload: Record<string, ColoursValue>) => void;
  open: boolean;
  ...
  const [value, setValue] = useState<Record<string, ColoursValue>>({
    ...initial,
...
function formatValue(value: ColoursValue, type: string) {
...
function defaultValue(type: string): ColoursValue {
...
function isFinalizedContactMaster(definition: ColoursDefinition) {
  return FINALIZED_COMMON_MASTER_KEYS.has(definition.key);
}
function isPlaceholder(record: ColoursRecord, definition: ColoursDefinition) {
...
export function ColoursWorkspace() {
  return <ColoursShell definition={coloursDefinition} />;
}
