import { bankNamesDefinition } from "./bank-names.types";
...
import type { BankNamesDefinition, BankNamesRecord, BankNamesValue } from "./bank-names.types";

export function BankNamesShell({ definition }: { definition: BankNamesDefinition }) {
  ...
  const [deleting, setDeleting] = useState<BankNamesRecord | null>(null);
  const query = useQuery({
    queryFn: () => listBankNames(definition.path),
    queryKey: ["core", "common", definition.key]
  });
  const save = useMutation({
    mutationFn: (payload: Record<string, BankNamesValue>) =>
      editing
        ? updateBankNames(definition.path, editing.id, payload)
        : createBankNames(definition.path, payload),
    onSuccess: async () => {
  ...
  const toggle = useMutation({
    mutationFn: (record: BankNamesRecord) =>
      setBankNamesActive(definition.path, record.id, !record.isActive),
    onSuccess: async () =>
      queryClient.invalidateQueries({ queryKey: ["core", "common", definition.key] })
  });
  const forceDelete = useMutation({
    mutationFn: (record: BankNamesRecord) => forceDeleteBankNames(definition.path, record.id),
...
export function BankNamesForm({
  ...
  loading: boolean;
  onClose: () => void;
  onSubmit: (payload: Record<string, BankNamesValue>) => void;
  open: boolean;
  ...
  const [value, setValue] = useState<Record<string, BankNamesValue>>({
    ...initial,
...
function formatValue(value: BankNamesValue, type: string) {
...
function defaultValue(type: string): BankNamesValue {
...
function isFinalizedContactMaster(definition: BankNamesDefinition) {
  return FINALIZED_COMMON_MASTER_KEYS.has(definition.key);
}
function isPlaceholder(record: BankNamesRecord, definition: BankNamesDefinition) {
...
export function BankNamesWorkspace() {
  return <BankNamesShell definition={bankNamesDefinition} />;
}
