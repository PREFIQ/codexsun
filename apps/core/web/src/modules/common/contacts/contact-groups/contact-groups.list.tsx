import { contactGroupsDefinition } from "./contact-groups.types";
...
export function ContactGroupsShell({ definition }: { definition: ContactGroupsDefinition }) {
  ...
  const [deleting, setDeleting] = useState<ContactGroupsRecord | null>(null);
  const query = useQuery({
    queryFn: () => listContactGroups(definition.path),
    queryKey: ["core", "common", definition.key]
  });
  const save = useMutation({
    mutationFn: (payload: Record<string, ContactGroupsValue>) =>
      editing
        ? updateContactGroups(definition.path, editing.id, payload)
        : createContactGroups(definition.path, payload),
    onSuccess: async () => {
  ...
  const toggle = useMutation({
    mutationFn: (record: ContactGroupsRecord) =>
      setContactGroupsActive(definition.path, record.id, !record.isActive),
    onSuccess: async () =>
      queryClient.invalidateQueries({ queryKey: ["core", "common", definition.key] })
  });
  const forceDelete = useMutation({
    mutationFn: (record: ContactGroupsRecord) =>
      forceDeleteContactGroups(definition.path, record.id),
...
export function ContactGroupsForm({
  ...
  loading: boolean;
  onClose: () => void;
  onSubmit: (payload: Record<string, ContactGroupsValue>) => void;
  open: boolean;
  ...
  const [value, setValue] = useState<Record<string, ContactGroupsValue>>({
    ...initial,
...
function formatValue(value: ContactGroupsValue, type: string) {
...
function defaultValue(type: string): ContactGroupsValue {
...
function isFinalizedContactMaster(definition: ContactGroupsDefinition) {
  return FINALIZED_COMMON_MASTER_KEYS.has(definition.key);
}
function isPlaceholder(record: ContactGroupsRecord, definition: ContactGroupsDefinition) {
...
export function ContactGroupsWorkspace() {
  return <ContactGroupsShell definition={contactGroupsDefinition} />;
}
