import { addressTypesDefinition } from "./address-types.types";
...
export function AddressTypesShell({ definition }: { definition: AddressTypesDefinition }) {
  ...
  const [deleting, setDeleting] = useState<AddressTypesRecord | null>(null);
  const query = useQuery({
    queryFn: () => listAddressTypes(definition.path),
    queryKey: ["core", "common", definition.key]
  });
  const save = useMutation({
    mutationFn: (payload: Record<string, AddressTypesValue>) =>
      editing
        ? updateAddressTypes(definition.path, editing.id, payload)
        : createAddressTypes(definition.path, payload),
    onSuccess: async () => {
  ...
  const toggle = useMutation({
    mutationFn: (record: AddressTypesRecord) =>
      setAddressTypesActive(definition.path, record.id, !record.isActive),
    onSuccess: async () =>
      queryClient.invalidateQueries({ queryKey: ["core", "common", definition.key] })
  });
  const forceDelete = useMutation({
    mutationFn: (record: AddressTypesRecord) => forceDeleteAddressTypes(definition.path, record.id),
...
export function AddressTypesForm({
  ...
  loading: boolean;
  onClose: () => void;
  onSubmit: (payload: Record<string, AddressTypesValue>) => void;
  open: boolean;
  ...
  const [value, setValue] = useState<Record<string, AddressTypesValue>>({
    ...initial,
...
function formatValue(value: AddressTypesValue, type: string) {
...
function defaultValue(type: string): AddressTypesValue {
...
function isFinalizedContactMaster(definition: AddressTypesDefinition) {
  return FINALIZED_COMMON_MASTER_KEYS.has(definition.key);
}
function isPlaceholder(record: AddressTypesRecord, definition: AddressTypesDefinition) {
...
export function AddressTypesWorkspace() {
  return <AddressTypesShell definition={addressTypesDefinition} />;
}
