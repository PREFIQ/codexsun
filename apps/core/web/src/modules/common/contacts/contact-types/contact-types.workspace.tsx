import { contactTypesDefinition } from "./contact-types.types";
...
export function ContactTypesShell({ definition }: { definition: ContactTypesDefinition }) {
  ...
  const [deleting, setDeleting] = useState<ContactTypesRecord | null>(null);
  const query = useQuery({
    queryFn: () => listContactTypes(definition.path),
    queryKey: ["core", "common", definition.key]
  });
  const save = useMutation({
    mutationFn: (payload: Record<string, ContactTypesValue>) =>
      editing
        ? updateContactTypes(definition.path, editing.id, payload)
        : createContactTypes(definition.path, payload),
    onSuccess: async () => {
  ...
  const toggle = useMutation({
    mutationFn: (record: ContactTypesRecord) =>
      setContactTypesActive(definition.path, record.id, !record.isActive),
    onSuccess: async () =>
      queryClient.invalidateQueries({ queryKey: ["core", "common", definition.key] })
  });
  const forceDelete = useMutation({
    mutationFn: (record: ContactTypesRecord) => forceDeleteContactTypes(definition.path, record.id),
...
export function ContactTypesForm({
  ...
  loading: boolean;
  onClose: () => void;
  onSubmit: (payload: Record<string, ContactTypesValue>) => void;
  open: boolean;
  ...
  const [value, setValue] = useState<Record<string, ContactTypesValue>>({
    ...initial,
...
function formatValue(value: ContactTypesValue, type: string) {
...
function defaultValue(type: string): ContactTypesValue {
...
function isFinalizedContactMaster(definition: ContactTypesDefinition) {
  return FINALIZED_COMMON_MASTER_KEYS.has(definition.key);
}
function isPlaceholder(record: ContactTypesRecord, definition: ContactTypesDefinition) {
...
export function ContactTypesWorkspace() {
  return <ContactTypesShell definition={contactTypesDefinition} />;
}
