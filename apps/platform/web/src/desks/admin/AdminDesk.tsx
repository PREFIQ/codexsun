      ...
      <AdminLayout
        ...
        actions={
          <div className="flex flex-wrap justify-end gap-2">
            ...
            <Button size="sm" variant={page === "app-registry" ? "default" : "ghost"} onClick={() => setPage("app-registry")}>Apps</Button>
        ...
        <main className="mx-auto w-[calc(100%-2rem)] max-w-[92rem] space-y-4 py-5 lg:w-[calc(100%-3rem)]">
          {page === "dashboard" ? (
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <AdminCard title="App Registry" icon={PanelsTopLeftIcon} text="Review platform applications and stack boundaries." />
      ...
      {(appsQuery.data ?? []).map((app) => (
        <Card key={app.id} title={app.label} description={app.description}>
          <div className="flex flex-wrap items-center gap-2">
            <StatusBadge tone={app.alwaysEnabled ? "green" : "blue"}>{app.alwaysEnabled ? "Always enabled" : "Switchable"}</StatusBadge>
            <StatusBadge tone="neutral">{app.stack}</StatusBadge>
            <span className="font-mono text-xs text-muted-foreground">{app.moduleKey}</span>
          </div>
        </Card>
      ))}
      {appsQuery.isLoading ? <Card title="App Registry" description="Loading registered apps."><StatusBadge tone="amber">Loading</StatusBadge></Card> : null}
    </div>
  );
}
