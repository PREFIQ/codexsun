...
const documents: Array<{ key: BillingDocumentKind; label: string; note: string }> = [
  { key: "quotation", label: "Quotation", note: "Quotation entry, item table, preview, and print controls." },
  { key: "sales", label: "Sales", note: "Sales invoice entry, item rows, E-invoice, and E-way controls." },
  { key: "purchase", label: "Purchase", note: "Purchase entry, supplier items, and GST transport controls." },
];
  ...
  const tabs: WorkspaceAnimatedTab[] = useMemo(() => [
    {
      ...
      value: "layout",
      content: (
        <SettingsPanel title="Billing Layout" note="Industry switches control fields independently for each billing document.">
          <div className="rounded-md border border-border/70 bg-background p-4">
            ...
            <div className="mt-2">
              <WorkspaceSelect
                ...
                onValueChange={(gstApiMode) => setForm((current) => ({
                  ...current,
    ...
    },
    {
      ...
      value: "printing",
      content: (
        <SettingsPanel title="Billing Printing" note="Shared print and letterhead controls for quotation, sales, and purchase.">
          <ToggleRow checked={form.printing.printWithLogo} label="Print with logo" note="Shows the active company logo in every billing document header." onChange={(printWithLogo) => patchPrinting({ printWithLogo })} badge="Client" />
          ...
          <LetterheadDesigner settings={form.printing.letterhead} showLogo={form.printing.printWithLogo} onChange={patchLetterhead} />
        </SettingsPanel>
      ),
    },
    {
      ...
      value: "customise",
      content: (
        <SettingsPanel title="Bill Customise" note="Shared document names, language, and totals presentation.">
            ...
            {documents.map((document) => (
              <SettingsField key={document.key} label={`${document.label} title`}>
                <Input
                  ...
                  onChange={(event) => patchCustomise({
                    documentTitles: { ...form.customise.documentTitles, [document.key]: event.target.value },
                  })}
                />
              </SettingsField>
            ))}
          ...
          <SettingsField label="Print language">
            <WorkspaceSelect value={form.customise.printLanguage} options={[{ label: "English", value: "english" }]} onValueChange={() => undefined} />
          </SettingsField>
        </SettingsPanel>
      ),
    },
    {
      ...
      value: "features",
      content: (
        <SettingsPanel title="Billing Features" note="Enable or disable complete billing document flows.">
          <ToggleRow checked={form.features.quotation} label="Quotation" note="Shows quotation entries and enables quotation consolidation into draft sales invoices." onChange={(quotation) => patchFeature("quotation", quotation)} badge="Client" />
          <ToggleRow checked={form.features.exportSales} label="Export Sales" note="Shows export sales entries, totals, shortcuts, and document settings." onChange={(exportSales) => patchFeature("exportSales", exportSales)} badge="Client" />
          <ToggleRow checked={form.features.tconnect} label="TConnect" note="Shows the TConnect trade connection workspace in the client app menu and landing desk." onChange={(tconnect) => patchFeature("tconnect", tconnect)} badge="Client" />
        </SettingsPanel>
      ),
    },
  ], [form]);
  ...
  return (
    <section className="space-y-4">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-normal text-foreground/80">Billing Settings</h1>
          <p className="mt-0.5 text-sm text-muted-foreground/70">Configure billing layout, printing, customisation, and feature controls.</p>
        </div>
        <Button
      ...
      {query.isError ? (
        <div className="rounded-md border border-destructive/30 bg-destructive/5 px-4 py-3 text-sm text-destructive">
          {query.error instanceof Error ? query.error.message : "Billing settings could not be loaded."}
        </div>
      ) : null}
...
function ToggleRow({ badge = "Industry", checked, disabled = false, label, note, onChange }: { badge?: "Client" | "Industry"; checked: boolean; disabled?: boolean; label: string; note: string; onChange: (checked: boolean) => void }) {
  return (
    <div className="flex min-h-16 items-center justify-between gap-3 rounded-md border border-border/70 bg-card px-4 py-3">
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className={badge === "Industry" ? "rounded border border-emerald-200 bg-emerald-50 px-2 py-0.5 text-[11px] text-emerald-700" : "rounded border border-sky-200 bg-sky-50 px-2 py-0.5 text-[11px] text-sky-700"}>{badge}</span>
        </div>
...
function SettingsPanel({ children, note, title }: { children?: React.ReactNode; note: string; title: string }) {
  return (
...
function LetterheadDesigner({ onChange, settings, showLogo }: { onChange: (next: Partial<BillingSettings["printing"]["letterhead"]>) => void; settings: BillingSettings["printing"]["letterhead"]; showLogo: boolean }) {
  ...
  return (
    <div className="rounded-md border border-border/70 bg-background p-4">
      <div className="relative overflow-hidden rounded-md border bg-white" style={{ borderColor: settings.borderColor, height: `${Math.max(settings.headerHeightMm * 3.6, 140)}px` }}>
        {showLogo ? (
          <div className="absolute flex items-center justify-center rounded-md border-2 border-neutral-700 text-lg font-bold text-neutral-700" style={{ height: `${settings.logoHeightMm * 3}px`, left: `${settings.logoLeftMm * 3}px`, top: `${settings.logoTopMm * 3}px`, width: `${settings.logoWidthMm * 3}px` }}>
            CS
          </div>
        ) : null}
        <div className="absolute inset-x-32 top-1/2 -translate-y-1/2 text-center">
          <div style={{ color: settings.companyColor, fontFamily: settings.companyFont, fontSize: `${settings.companySize}px`, fontWeight: 700 }}>CODEXSUN</div>
          <div style={{ color: settings.addressColor, fontFamily: settings.addressFont, fontSize: `${settings.addressSize}px` }}>address1, ADDRESS 2</div>
          <div style={{ color: settings.addressColor, fontFamily: settings.addressFont, fontSize: `${settings.contactSize}px` }}>Tiruppur, Tamil Nadu, India - 641602</div>
        </div>
      </div>
      <div className="mt-4">
        <h3 className="text-sm font-semibold">Letterhead Designer</h3>
        <p className="mt-1 text-sm text-muted-foreground">Used by quotation, sales, purchase, receipt, payment, stock documents, and statements.</p>
      </div>
      <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        {fields.map((field) => (
          <label key={field.key} className="text-sm text-muted-foreground">
            ...
            <Input
              className={field.type === "color" ? "h-10 w-14 cursor-pointer rounded-md p-1" : "h-10 rounded-md"}
