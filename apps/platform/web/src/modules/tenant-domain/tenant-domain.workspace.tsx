...
export function TenantDomainList() {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: async (domain) => {
      await invalidateDomainData(queryClient);
      toast.success("Domain updated", { description: `${domain.domain} was updated successfully.` });
      setView({ domain, mode: "show" });
    }
  });
  ...
  const filteredDomains = useMemo(() => {
    ...
    return domains.filter((domain) => {
      const matchesSearch =
        !term ||
        [domain.domain, domain.uuid, domain.tenantName, domain.tenantCode, domain.tenantStatus].some((value) =>
  ...
  if (view.mode === "upsert") {
    return (
      <DomainUpsertPage
        ...
        onBack={() =>
          setView(view.returnTo === "show" && view.domain ? { domain: view.domain, mode: "show" } : { mode: "list" })
        }
        onSubmit={(payload) => {
          if (view.domain) {
  ...
  return (
    ...
    >
      ...
      <WorkspaceTablePanel>
          ...
          <table className="w-full min-w-[780px] border-collapse text-sm">
            <thead className="bg-muted/50">
              <tr>
                ...
                {visibleColumns.domain ? <WorkspaceTableHeaderCell>Domain</WorkspaceTableHeaderCell> : null}
                {visibleColumns.uuid ? <WorkspaceTableHeaderCell>UUID</WorkspaceTableHeaderCell> : null}
                {visibleColumns.tenant ? <WorkspaceTableHeaderCell>Tenant</WorkspaceTableHeaderCell> : null}
                {visibleColumns.tenantCode ? <WorkspaceTableHeaderCell>Tenant code</WorkspaceTableHeaderCell> : null}
                {visibleColumns.primary ? <WorkspaceTableHeaderCell>Type</WorkspaceTableHeaderCell> : null}
                {visibleColumns.status ? <WorkspaceTableHeaderCell>Tenant status</WorkspaceTableHeaderCell> : null}
            ...
            <tbody>
              {pageDomains.map((domain, index) => (
                <tr className="border-b border-border/70 last:border-b-0" key={domain.id}>
                  ...
                  {visibleColumns.uuid ? <td className="px-4 py-2.5 font-mono text-xs text-muted-foreground">{domain.uuid}</td> : null}
                  {visibleColumns.tenant ? <td className="px-4 py-2.5 font-medium">{domain.tenantName}</td> : null}
                  {visibleColumns.tenantCode ? <td className="px-4 py-2.5 font-mono text-xs text-muted-foreground">{domain.tenantCode}</td> : null}
                  ...
                  {visibleColumns.status ? (
                    <td className="px-4 py-2.5">
                      <WorkspaceStatusBadge label={domain.tenantStatus} tone={statusTone(domain.tenantStatus)} />
                    </td>
                  ) : null}
        ...
        {pageDomains.length === 0 && domainsQuery.isFetching ? <WorkspaceTableSkeletonRows columns={8} /> : null}
      ...
      <WorkspaceShowLayout>
        <WorkspaceShowCard title="Domain details">
          <WorkspaceDetailTable
            rows={[
              ["Domain", <span className="font-mono text-xs" key="domain">{domain.domain}</span>],
              ["UUID", <span className="font-mono text-xs" key="uuid">{domain.uuid}</span>],
              ["Type", domain.isPrimary ? "Primary" : "Alias"],
              ["Tenant", domain.tenantName],
              ["Tenant code", <span className="font-mono text-xs" key="code">{domain.tenantCode}</span>],
        ...
        <WorkspaceShowCard title="Routing">
          <div className="flex items-start gap-3 p-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">Routes requests to {domain.tenantName}.</p>
            </div>
          </div>
        </WorkspaceShowCard>
      </WorkspaceShowLayout>
        ...
        <WorkspaceFormPanel
          footer={
            <WorkspaceFormFooter
              ...
              primaryProps={{ children: <><Save className="size-4" />{domain ? "Save changes" : "Create domain"}</> }}
            />
          }
          title="Domain identity"
          description="Domains are normalized automatically and saved as the tenant's primary route."
        >
          {error ? <WorkspaceFormBanner title="Unable to save domain">{error.message}</WorkspaceFormBanner> : null}
          <WorkspaceFormGrid>
