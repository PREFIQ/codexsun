...
import { BanIcon, CheckCircle2Icon, Clock3Icon, Layers3Icon, PlayIcon, RotateCcwIcon, TriangleAlertIcon } from "lucide-react";
  ...
  return (
    <section className="space-y-4">
      <div className="grid gap-4 md:grid-cols-5">
        <MetricCard icon={Layers3Icon} label="Backend" value={settings?.backendLabel ?? "Database queue"} />
        <MetricCard icon={Clock3Icon} label="Pending" value={String(pending)} />
        <MetricCard icon={PlayIcon} label="Running" value={String(running)} />
        <MetricCard icon={TriangleAlertIcon} label="Failed" value={String(failed)} {...(failed > 0 ? { tone: "red" as const } : {})} />
        <MetricCard icon={CheckCircle2Icon} label="Completed" value={String(completed)} tone="green" />
      </div>
      <div className="rounded-md border bg-card p-4 shadow-sm">
        <div className="grid gap-3 md:grid-cols-4">
          ...
          <WorkflowStep active={failed > 0} count={failed} label="Needs review" {...(failed > 0 ? { tone: "red" as const } : {})} />
              ...
              <tr className="border-t bg-muted/20 align-top" key={`${job.uuid}-details`}>
                <td className="px-4 py-3 text-xs text-muted-foreground" colSpan={8}>
                  <div className="grid gap-3 md:grid-cols-2">
                    <pre className="max-h-28 overflow-auto rounded-md border bg-background p-3"><span className="mb-2 block font-medium text-foreground">Payload</span>{JSON.stringify(maskDetails(job.payload), null, 2)}</pre>
                    <pre className="max-h-28 overflow-auto rounded-md border bg-background p-3"><span className="mb-2 block font-medium text-foreground">Result</span>{JSON.stringify(maskDetails(job.result), null, 2)}</pre>
                  </div>
                </td>
              </tr>
            ))}
...
function MetricCard({ icon: Icon, label, tone, value }: { icon: typeof Layers3Icon; label: string; tone?: "green" | "red" | undefined; value: string }) {
  return (
    <div className="rounded-md border bg-card p-5 shadow-sm">
      <Icon className="size-5 text-muted-foreground" />
      <div className="mt-5 text-xl font-semibold">{tone ? <StatusBadge tone={tone}>{value}</StatusBadge> : value}</div>
      <div className="mt-1 text-sm text-muted-foreground">{label}</div>
    </div>
  );
}

function WorkflowStep({ active, count, label, tone }: { active: boolean; count: number; label: string; tone?: "green" | "red" | undefined }) {
  return (
