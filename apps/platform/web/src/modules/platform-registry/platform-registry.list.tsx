...
export function PlatformList(props: Omit<RegistryListProps<ProjectManagerRegistryPlatformNode>, "emptyLabel" | "itemLabel">) {
  return <RegistryTable {...props} emptyLabel="No platforms found." itemLabel="Platform" />;
}

export function ModuleGroupList(props: Omit<RegistryListProps<ProjectManagerRegistryGroupNode>, "emptyLabel" | "itemLabel">) {
  return <RegistryTable {...props} emptyLabel="No module groups found." itemLabel="Module Group" />;
}

export function ModuleList(props: Omit<RegistryListProps<ProjectManagerRegistryModuleNode>, "emptyLabel" | "itemLabel">) {
  return <RegistryTable {...props} emptyLabel="No modules found." itemLabel="Module" />;
}

export function ModuleGroupModuleList(props: Omit<RegistryListProps<ProjectManagerRegistryModuleNode>, "emptyLabel" | "itemLabel">) {
  return <RegistryTable {...props} emptyLabel="No module groups found." itemLabel="Module Group" />;
}

export function AppList(props: Omit<RegistryListProps<ProjectManagerRegistryModuleNode>, "emptyLabel" | "itemLabel">) {
  return <RegistryTable {...props} emptyLabel="No apps found." itemLabel="App" />;
}

export function SubmoduleList(props: Omit<RegistryListProps<ProjectManagerRegistryModuleNode>, "emptyLabel" | "itemLabel">) {
  return <RegistryTable {...props} emptyLabel="No submodules found." itemLabel="Submodule" />;
}

function RegistryTable<T extends RegistryItem>({ emptyLabel, itemLabel, items, onEdit, onSelect, onToggle }: RegistryListProps<T>) {
  return (
    <WorkspaceTablePanel>
          ...
          {items.map((item, index) => (
            <tr className="border-b border-border/70 last:border-b-0" key={item.id}>
              ...
              <td className="px-4 py-2.5">
                <button className="font-medium hover:underline" onClick={() => onSelect(item)} type="button">{item.name}</button>
              ...
              <td className="px-4 py-2.5"><WorkspaceStatusBadge label={item.active ? item.status : "off"} tone={item.active ? "success" : "danger"} /></td>
              <td className="px-4 py-2.5">
      ...
      {items.length === 0 ? <WorkspaceTableEmptyState>{emptyLabel}</WorkspaceTableEmptyState> : null}
    </WorkspaceTablePanel>
  );
}
