...
import { useStorageListingQuery, useStorageMutations, useStorageRootsQuery } from "./storage-manager.hooks";
...
export function StorageManagerWorkspace() {
  ...
  const [state, setState] = useState<StorageBrowserState>({ path: "", scope: "app", tenantId: firstTenantId, visibility: "public" });
  const resolvedState = useMemo(() => cleanStorageState({ ...state, tenantId: state.tenantId || firstTenantId }), [firstTenantId, state]);
  ...
  const busy = roots.isLoading || listing.isLoading || mutations.createFolder.isPending || mutations.download.isPending || mutations.upload.isPending;
    ...
    <WorkspacePage
      ...
      actions={
        <StorageManagerForm
          ...
          onRefresh={() => { void roots.refetch(); void listing.refetch(); }}
