...
import { getTenantDatabaseDetails, listTenantDatabaseStatus, migrateTenantDatabase, requestTenantDatabaseBackup, requestTenantDatabaseRestore } from "./tenant-database.services";

export const tenantDatabaseQueryKey = ["admin", "database", "tenants"] as const;

export function useTenantDatabaseQuery() {
  return useQuery({ queryFn: listTenantDatabaseStatus, queryKey: tenantDatabaseQueryKey, meta: { suppressGlobalLoader: true }, refetchInterval: 15_000 });
}
