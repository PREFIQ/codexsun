...
export async function registerQueueManagerRoutes(app: FastifyInstance) {
  app.get("/admin/queue/settings", { preHandler: requireSuperAdmin }, async (request) => ok(await service.runtimeSettings(), { requestId: request.id }));
  app.get("/admin/queue/jobs", { preHandler: requireSuperAdmin }, async (request) => ok(await service.listJobs(filtersFromQuery(request.query)), { requestId: request.id }));
  ...
  app.post("/admin/queue/jobs/:id/run", { preHandler: requireSuperAdmin }, async (request, reply) => {
    const job = await service.runJob(Number((request.params as { id: string }).id));
    if (!job) return reply.code(404).send(notFound("QUEUE_JOB_NOT_FOUND", "Queue job was not found.", request.id));
    return ok(job, { requestId: request.id });
  });
  app.post("/admin/queue/jobs/:id/retry", { preHandler: requireSuperAdmin }, async (request, reply) => {
    const job = await service.retryJob(Number((request.params as { id: string }).id));
    if (!job) return reply.code(404).send(notFound("QUEUE_JOB_NOT_FOUND", "Queue job was not found.", request.id));
    return ok(job, { requestId: request.id });
  });
  app.post("/admin/queue/jobs/:id/cancel", { preHandler: requireSuperAdmin }, async (request, reply) => {
    const job = await service.cancelJob(Number((request.params as { id: string }).id));
    if (!job) return reply.code(404).send(notFound("QUEUE_JOB_NOT_FOUND", "Queue job was not found.", request.id));
    return ok(job, { requestId: request.id });
  });
  app.post("/admin/queue/cleanup", { preHandler: requireSuperAdmin }, async (request) => ok(await service.cleanupRetainedJobs(), { requestId: request.id }));
}
...
function filtersFromQuery(query: unknown): QueueJobFilters {
  ...
  const status = typeof input.status === "string" && ["cancelled", "completed", "failed", "pending", "running"].includes(input.status) ? input.status as QueueJobStatus : undefined;
  return {
    ...(typeof input.correlationId === "string" && input.correlationId.trim() ? { correlationId: input.correlationId.trim() } : {}),
    ...(typeof input.queueName === "string" && input.queueName.trim() ? { queueName: input.queueName.trim() } : {}),
    ...(status ? { status } : {}),
