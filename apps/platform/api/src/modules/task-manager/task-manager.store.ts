...
export class TaskManagerJsonStore {
  ...
  async create(tenantKey: string, input: TodoInput) { const title = input.title.trim(); if (!title) throw AppError.validation("Todo title is required."); const now = new Date().toISOString(); const records = await this.read(tenantKey); const record: Todo = { id: `todo-${randomUUID().slice(0, 8)}`, title, description: String(input.description ?? ""), status: input.status ?? "open", priority: input.priority ?? "medium", dueDate: String(input.dueDate ?? ""), position: records.length, createdAt: now, updatedAt: now }; records.push(record); await this.write(tenantKey, records); return record; }
  ...
  private async read(tenantKey: string) { const file = fileFor(tenantKey); await ensure(file); const records = JSON.parse(await readFile(file, "utf8")) as Array<Partial<Todo>>; return records.map((item, index) => ({ ...item, position: typeof item.position === "number" ? item.position : index })) as Todo[]; }
