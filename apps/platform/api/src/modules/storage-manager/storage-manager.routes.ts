...
export async function registerStorageManagerRoutes(app: FastifyInstance) {
  app.get("/admin/storage/roots", { preHandler: requireSuperAdmin }, async (request) => ok(await service.roots(), { requestId: request.id }));
  app.get("/admin/storage/list", { preHandler: requireSuperAdmin }, async (request) => ok(await service.list(storageListFromQuery(request.query)), { requestId: request.id }));
  app.post("/admin/storage/folders", { preHandler: requireSuperAdmin }, async (request) => ok(await service.createFolder(request.body as StorageFolderPayload), { requestId: request.id }));
  app.post("/admin/storage/upload", { preHandler: requireSuperAdmin }, async (request) => ok(await service.upload(request.body as StorageUploadPayload), { requestId: request.id }));
  app.get("/admin/storage/download", { preHandler: requireSuperAdmin }, async (request, reply) => sendDownload(reply, await service.download(storageDownloadFromQuery(request.query))));
...
async function requireTenantUser(request: FastifyRequest, reply: FastifyReply) {
  ...
  return reply.code(403).send(fail({ code: "TENANT_STORAGE_REQUIRED", message: "Tenant storage access requires a tenant session." }, { requestId: request.id }));
}
...
function tenantStorageInput(request: FastifyRequest, value: unknown): StorageListInput & { file?: string } {
...
function storageListFromQuery(value: unknown): StorageListInput {
  ...
  return {
    ...
    scope,
    tenantId: typeof input.tenantId === "string" || typeof input.tenantId === "number" ? input.tenantId : null,
    visibility
  };
}
...
function sendDownload(reply: FastifyReply, file: Awaited<ReturnType<StorageManagerService["download"]>>) {
  return reply
