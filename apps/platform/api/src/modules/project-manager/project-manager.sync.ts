...
export function shouldSyncProjectManagerRecord(record: Pick<ProjectManagerRecord, "active" | "status">) {
  return record.active && record.status !== "archived";
}

export function buildProjectManagerSyncSummary(records: Array<Pick<ProjectManagerRecord, "active" | "status">>) {
