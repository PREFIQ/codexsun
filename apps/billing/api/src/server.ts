  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      const address = await app.listen({
        host: env.BILLING_API_HOST,
        port: env.BILLING_API_PORT
      });
      console.log(`  ok Billing API ready: ${address}`);
      return;
    } catch (error) {
      if (isAddressInUseError(error) && attempt < retries) {
        app.log.warn(`Port ${env.BILLING_API_PORT} in use, retrying in ${delay}ms... (attempt ${attempt}/${retries})`);
