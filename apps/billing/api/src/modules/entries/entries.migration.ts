...
type Database = Kysely<BillingDatabase>;
...
async function migrateEntryTables(database: Database, kind: "quotation" | "sales" | "purchase" | "exportSales") {
  ...
  `).execute(database);
  await addColumnIfMissing(database, headerTable, "supplier_bill_no", "VARCHAR(120) NULL");
  await addColumnIfMissing(database, headerTable, "supplier_bill_date", "DATE NULL");
...
async function addColumnIfMissing(database: Database, tableName: string, columnName: string, definition: string) {
