...
import type { ExportSale, ExportSaleSavePayload, ExportSaleStatus } from "./export-sales.types.js";

type ExportSalesTableRow = {
  amount: number;
...
type ExportSalesDatabase = {
  billing_export_sales: ExportSalesTableRow;
};
...
function toExportSale(row: ExportSalesTableRow): ExportSale {
...
function toExportSaleRow(sale: ExportSale): ExportSalesTableRow {
  return {
    amount: sale.amount,
...
function createExportSaleRecord(input: ExportSaleSavePayload, current?: Partial<Pick<ExportSale, "createdAt" | "id">>) {
  ...
  return {
    ...
    id: current?.id ?? `export-sale-${randomUUID().slice(0, 8)}`,
