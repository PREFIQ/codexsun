...
export class ReceiptService {
  ...
  async create(databaseName: string, input: ReceiptInput) {
    ...
    const number = String(input.receiptNumber ?? "").trim() || generated;
  ...
  async setStatus(databaseName: string, id: string, status: ReceiptStatus) {
    ...
    if (status === "posted" && current.status !== "draft")
      throw AppError.conflict("Only draft receipts can be posted.");
  ...
  private assertDraft(status: ReceiptStatus) {
    if (status !== "draft") throw AppError.conflict("Only draft receipts can be edited.");
  }
}
