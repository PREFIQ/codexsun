...
import type { Payment, PaymentInput, PaymentStatus } from "./payment.types.js";
...
export class PaymentRepository {
  ...
  async create(databaseName: string, input: PaymentInput) {
    ...
    const db = (await getBillingDatabase(databaseName)) as any;
    await db.insertInto("billing_payments").values(toRow(record)).execute();
    return record;
  }
  async update(databaseName: string, id: string, input: PaymentInput) {
    ...
    const record = makePayment(input, id, current.created_at);
    await db.updateTable("billing_payments").set(toRow(record)).where("id", "=", id).execute();
    return record;
  }
  ...
  async delete(databaseName: string, id: string) {
    ...
    const db = (await getBillingDatabase(databaseName)) as any;
    await db.deleteFrom("billing_payments").where("id", "=", id).execute();
    return record;
  }
}
...
function makePayment(
  ...
  return {
    id,
    paymentNumber: String(input.paymentNumber ?? ""),
    paymentDate: input.paymentDate,
    partyName: input.partyName.trim(),
    partyId: String(input.partyId ?? ""),
    partyType: String(input.partyType ?? "supplier"),
    ...
    status: (input.status ?? "draft") as PaymentStatus,
