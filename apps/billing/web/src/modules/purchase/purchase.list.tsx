...
export function PurchaseList({
  ...
  entries: Purchase[];
  loading: boolean;
  onEdit: (sale: Purchase) => void;
  onPrint: (sale: Purchase) => void;
  onSetStatus: (sale: Purchase, status: "cancelled" | "confirmed") => void;
  onView: (sale: Purchase) => void;
  rowsPerPage: number;
          ...
          <tbody>
            {entries.map((sale, index) => (
              <tr key={sale.id} className={cn("border-b border-border/70 last:border-b-0", sale.status === "cancelled" && "bg-muted/20 text-muted-foreground")}>
                ...
                {visibleColumns.items ? <td className="px-4 py-2.5 tabular-nums">{totalPurchaseQuantity(sale)}</td> : null}
...
function statusTone(status: Purchase["status"]) {
