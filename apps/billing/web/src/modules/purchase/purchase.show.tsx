...
export function PurchaseShowPage({
  ...
  const activityItems = useMemo(
    () => [
      ...toolActivities,
      { createdAt: purchase.updatedAt, id: "updated", message: `Purchase updated${purchase.generatedSalesInvoiceNo ? ` and linked to ${purchase.generatedSalesInvoiceNo}` : ""}` },
  ...
  return (
    ...
    >
      <main className="mx-auto w-full pb-8">
        <div className="mb-4 grid gap-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="order-1 flex flex-wrap items-center justify-end gap-2 sm:order-2">
              ...
              <Button disabled={!canEdit} title={canEdit ? "Edit purchase" : "Submitted purchases cannot be edited"} type="button" variant="outline" className="rounded-xl" onClick={onEdit}><Pencil className="size-4" />Edit</Button>
        ...
        <section className="grid items-start gap-4 py-2 xl:grid-cols-[minmax(0,1fr)_15rem]">
            ...
            <CardContent className="space-y-8">
              ...
              {comments.length || purchase.notes ? (
                <div className="space-y-2">
                  {purchase.notes ? <SideNote body={purchase.notes} meta="Saved notes" title="System" /> : null}
                ...
                <div key={tool.id} className="border-b border-border/70 last:border-b-0">
                  ...
                  {openTool === tool.id ? (
                    <div className="px-3 pb-3">
                      {tool.id === "email" ? <InlineSend value={emailAddress} placeholder="Email address" onChange={setEmailAddress} onSend={() => { const value = emailAddress.trim(); if (!value) return; recordActivity(`Queued purchase email to ${value}`); setEmailAddress(""); }} /> : null}
                      {tool.id === "assign" ? <Input value={assigneeInput} onChange={(event) => setAssigneeInput(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); addListValue(assigneeInput, setAssigneeInput, setAssignees, (value) => `Assigned purchase to ${value}`); } }} placeholder="User name or email" className="h-9 rounded-md" /> : null}
