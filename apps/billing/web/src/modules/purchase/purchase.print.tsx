...
export function PurchasePrintRoutePage() {
  ...
  const saleQuery = usePurchaseRecord(saleId, true);
  const sale = saleQuery.data;

  return (
    <WorkspacePage
      title={sale ? `${sale.invoiceNumber} print` : "Purchase print"}
      description="Printable bill surface for the purchase workspace."
      actions={
    ...
    >
      <PageTitle title={sale ? `${sale.invoiceNumber} Print` : "Purchase Print"} />
      {sale ? <PurchasePrintDocument sale={sale} /> : <div className="rounded-md border border-border/70 bg-card/95 px-4 py-8 text-sm text-muted-foreground">{saleQuery.isLoading ? "Loading purchase print view..." : "Purchase print record was not found."}</div>}
    </WorkspacePage>
  );
}

export function PurchasePrintDocument({ sale }: { sale: Purchase }) {
  return (
    <WorkspacePrintSheet>
      <article className="border border-slate-300 bg-white p-6 text-[10px] text-slate-900 shadow-sm">
        <header className="grid gap-4 border-b border-slate-300 pb-4 md:grid-cols-[1fr_auto]">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-semibold tracking-tight">Purchase Bill</h1>
            <p className="mt-2 max-w-xl text-sm leading-6 text-slate-600">
              Purchase document prepared from the billing workspace with the same structured invoice rhythm used in the reference system.
            </p>
          </div>
        ...
        <section className="mt-6 grid gap-4 md:grid-cols-[1fr_19rem]">
          <div className="rounded-md border border-slate-300 px-4 py-4">
            ...
            <p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-slate-700">{sale.notes || "No notes added for this purchase bill."}</p>
          </div>
          <div className="rounded-md border border-slate-300 px-4 py-4">
        ...
        <footer className="mt-8 grid gap-4 border-t border-slate-300 pt-4 md:grid-cols-[1fr_16rem]">
          <div className="text-[9px] leading-5 text-slate-500">
            Generated from the CODEXSUN billing purchase workspace. Review the record activity in the app before dispatching the bill externally.
          </div>
