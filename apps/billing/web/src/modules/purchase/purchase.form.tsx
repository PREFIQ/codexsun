...
import { formatMoney } from "./purchase.services";
import { purchaseSchema } from "./purchase.schema";
import { createEmptyPurchase, createEmptyPurchaseItem, type Purchase, type PurchaseLineItemInput, type PurchaseSavePayload } from "./purchase.types";

export function PurchaseUpsertPage({
  errorMessage,
  loading,
  onBack,
  onSubmit,
  ...
  loading: boolean;
  onBack: () => void;
  onSubmit: (payload: PurchaseSavePayload) => void;
  ...
  const [draftItem, setDraftItem] = useState<PurchaseLineItemInput>(createEmptyPurchaseItem());
  ...
  const tabs: WorkspaceAnimatedTab[] = [
      ...
      content: (
        <WorkspaceFormPanel>
          <WorkspaceFormGrid columns={2}>
            ...
            <WorkspaceFormField label="Purchase number" required>
              <Input
            ...
            <WorkspaceFormField label="Supplier bill no">
              <Input className="h-11 rounded-md uppercase" value={form.supplierBillNo} onChange={(event) => updateField("supplierBillNo", event.target.value.toUpperCase())} />
            </WorkspaceFormField>
            <WorkspaceFormField label="Purchase tax type" required>
              <WorkspaceSelect
                ariaLabel="Purchase tax type"
                options={[
            ...
            <WorkspaceFormField label="Supplier bill date">
              <WorkspaceDatePicker value={form.supplierBillDate} onValueChange={(value) => updateField("supplierBillDate", value)} />
            </WorkspaceFormField>
          </WorkspaceFormGrid>
        </WorkspaceFormPanel>
      ),
    ...
    },
    {
      ...
      value: "items",
      content: (
        <WorkspaceFormPanel title="Purchase Items">
              ...
              <table className="w-full min-w-[1100px] border-collapse text-sm">
                ...
                <tbody>
                  {form.items.map((item, index) => {
                    ...
                    return (
                      <tr key={`purchase-item-${index}`} className="border-t border-border/70">
                        ...
                        <td className="px-3 py-3 text-right">{formatMoney(halfGst, form.currencyCode)}</td>
                        <td className="px-3 py-3 text-right">{formatMoney(halfGst, form.currencyCode)}</td>
          ...
          <WorkspaceFormGrid columns={2}>
            <WorkspaceFormField label="Status" required>
              <WorkspaceSelect
                ...
                value={form.status}
                onValueChange={(value) => updateField("status", value as PurchaseSavePayload["status"])}
              />
            </WorkspaceFormField>
  ...
  function updateField<K extends keyof PurchaseSavePayload>(key: K, value: PurchaseSavePayload[K]) {
    setBanner("");
    setForm((current) => ({ ...current, [key]: value }));
  }

  function updateDraft<K extends keyof PurchaseLineItemInput>(key: K, value: PurchaseLineItemInput[K]) {
    setBanner("");
  ...
  function editItem(index: number) {
    setDraftItem({ ...createEmptyPurchaseItem(), ...form.items[index] });
  ...
  return (
    <WorkspaceUpsertPage
      ...
      description="Create and update purchase bills in the same billing tone as the live entry desk."
      action={
    ...
    >
      <form
        ...
        onSubmit={(event) => {
          ...
          if (!parsed.success) {
            setBanner(parsed.error.issues[0]?.message ?? "Please complete the required purchase fields.");
            return;
          }
          setBanner("");
...
function toPurchasePayload(purchase: Purchase): PurchaseSavePayload {
  return {
...
function calculateItemTotal(item: PurchaseLineItemInput) {
...
function buildDraftTotals(form: PurchaseSavePayload) {
