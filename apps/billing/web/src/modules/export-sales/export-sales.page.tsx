...
import { createEmptyExportSale, type ExportSale, type ExportSaleSavePayload, type ExportSaleTaxType, type ExportSaleView } from "./export-sales.types";
...
export function ExportSalePage() {
  ...
  return (
    <BillingLayout
      ...
      headerTitle="ExportSale"
      subtitle="Create, review, print, and convert tenant-isolated exportSale vouchers."
      title="Billing Workspace"
    >
      <PageTitle title="ExportSale" />
      <ExportSaleWorkspace />
    </BillingLayout>
  );
}
...
export function ExportSaleWorkspace() {
  ...
  const [visibleColumns, setVisibleColumns] = useState<Record<string, boolean>>(() => Object.fromEntries(exportSaleColumnCatalog.map((column) => [column.id, true])));
  ...
  const saveMutation = useMutation({
    ...
    onError: (error) => {
      toast.error("ExportSale save failed", { description: error instanceof Error ? error.message : "Please try again." });
    },
  });
  ...
  const statusMutation = useMutation({
    ...
    onSuccess: async (exportSale) => {
      ...
      toast.success("ExportSale status updated", { description: `${exportSale.invoiceNumber} is now ${exportSale.status}.` });
  ...
  const revokeMutation = useMutation({
    ...
    onSuccess: async (exportSale) => {
      await queryClient.invalidateQueries({ queryKey: ["billing", "exportSales"] });
      toast.success("ExportSale revoked", { description: `${exportSale.invoiceNumber} is editable again.` });
    },
    onError: (error) => {
      toast.error("ExportSale revoke failed", { description: error instanceof Error ? error.message : "Please try again." });
    },
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: async (exportSale) => {
      await queryClient.invalidateQueries({ queryKey: ["billing", "exportSales"] });
      toast.success("ExportSale deleted", { description: exportSale.invoiceNumber });
    },
    onError: (error) => {
      toast.error("ExportSale could not be deleted", { description: error instanceof Error ? error.message : "Only draft exportSales can be deleted." });
    },
  });
  ...
  const filteredEntries = useMemo(() => {
    ...
    return entries.filter((exportSale) => {
      ...
      const matchesContact = contactFilter === "all" || exportSaleContactKey(exportSale) === contactFilter;
  ...
  const selectedEntries = useMemo(() => entries.filter((exportSale) => selectedExportSaleIds.has(exportSale.id)), [entries, selectedExportSaleIds]);
  const pageSelectableEntries = pageEntries.filter(canGenerateInvoiceFromExportSale);
  const pageSelected = pageSelectableEntries.length > 0 && pageSelectableEntries.every((exportSale) => selectedExportSaleIds.has(exportSale.id));
  ...
  async function openNewExportSale() {
  ...
  function toggleExportSaleSelection(exportSale: ExportSale, checked: boolean) {
    if (!canGenerateInvoiceFromExportSale(exportSale)) return;
    setSelectedExportSaleIds((current) => {
  ...
  if (view.mode === "show") {
    const freshExportSale = entries.find((entry) => entry.id === view.exportSale.id) ?? view.exportSale;
    ...
    const nextExportSale = currentIndex >= 0 && currentIndex < entries.length - 1 ? entries[currentIndex + 1] : null;
    return (
      <ExportSaleShowPage
        ...
        {...(previousExportSale ? { onPrevious: () => setView({ mode: "show", exportSale: previousExportSale }) } : {})}
        {...(nextExportSale ? { onNext: () => setView({ mode: "show", exportSale: nextExportSale }) } : {})}
      />
    );
  }
  ...
  if (view.mode === "upsert") {
    return (
        ...
        {...(view.exportSale && canAdminRevoke ? { onRevoke: () => revokeMutation.mutate(view.exportSale!.id) } : {})}
        onBack={() => setView(view.returnTo === "show" && view.exportSale ? { mode: "show", exportSale: view.exportSale } : { mode: "list" })}
  ...
  return (
    <WorkspacePage
      title="ExportSales"
      description="Create and review tenant-isolated exportSale vouchers with exportSales layout controls."
      technicalName="page.billing.exportSale.list"
      actions={
        <div className="flex flex-wrap items-center gap-2">
          <Button className="h-9 rounded-md" disabled={exportSalesQuery.isFetching} onClick={() => void exportSalesQuery.refetch()} type="button" variant="outline">
    ...
    >
      <WorkspaceFilters
        ...
        searchPlaceholder="Search exportSale, customer, work order, date, or total"
        searchValue={searchValue}
        columnOptions={exportSaleColumnCatalog.map((column) => ({ ...column, checked: Boolean(visibleColumns[column.id]), onCheckedChange: (checked: boolean) => setVisibleColumns((current) => ({ ...current, [column.id]: checked })) }))}
        onShowAllColumns={() => setVisibleColumns(Object.fromEntries(exportSaleColumnCatalog.map((column) => [column.id, true])))}
      />
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-md border border-border/70 bg-card px-4 py-3 text-sm shadow-sm">
        ...
        <div className="flex items-center gap-2 text-muted-foreground">
          ...
          <Button className="h-8 rounded-md px-2" disabled={!selectedEntries.length} onClick={() => setSelectedExportSaleIds(new Set())} type="button" variant="ghost">Clear</Button>
        </div>
      </div>
      {exportSalesQuery.isError ? (
        <WorkspaceTablePanel>
          <WorkspaceTableEmptyState>{exportSalesQuery.error instanceof Error ? exportSalesQuery.error.message : "ExportSales could not be loaded."}</WorkspaceTableEmptyState>
        </WorkspaceTablePanel>
      ) : null}
      <ExportSaleList
        entries={pageEntries}
        ...
        onForceDelete={(exportSale) => {
          if (window.confirm(`Force delete ${exportSale.invoiceNumber}? This cannot be undone.`)) deleteMutation.mutate(exportSale.id);
        }}
      ...
      />
      <ExportSalePageTotals amount={pageTotals.amount} quantity={pageTotals.quantity} subtotal={pageTotals.subtotal} taxAmount={pageTotals.taxAmount} />
      <WorkspacePagination
        page={currentPage}
        rowsPerPage={rowsPerPage}
        showingLabel={buildShowingLabel(currentPage, rowsPerPage, filteredEntries.length)}
        singularLabel="exportSales"
...
function ExportSalePageTotals({ amount, quantity, subtotal, taxAmount }: { amount: number; quantity: number; subtotal: number; taxAmount: number }) {
  return (
...
export const ExportSalesPage = ExportSalePage;
...
function ExportSaleList({ canAdminRevoke, entries, loading, onEdit, onForceDelete, onRevoke, onSetStatus, onView, page, pageSelected, pageSelectableCount, rowsPerPage, selectedExportSaleIds, onTogglePageSelection, onToggleSelection, visibleColumns }: { canAdminRevoke: boolean; entries: ExportSale[]; loading: boolean; onEdit: (exportSale: ExportSale) => void; onForceDelete: (exportSale: ExportSale) => void; onRevoke: (exportSale: ExportSale) => void; onSetStatus: (exportSale: ExportSale, status: "cancelled" | "confirmed") => void; onView: (exportSale: ExportSale) => void; page: number; pageSelected: boolean; pageSelectableCount: number; rowsPerPage: number; selectedExportSaleIds: Set<string>; onTogglePageSelection: (checked: boolean) => void; onToggleSelection: (exportSale: ExportSale, checked: boolean) => void; visibleColumns: Record<string, boolean> }) {
  return (
    <WorkspaceTablePanel>
      <div className="overflow-x-auto">
        <table className="w-full min-w-[980px] border-collapse text-sm">
          <thead className="bg-muted/50">
            <tr>
              <th className="w-12 border-b border-border/70 px-4 py-3.5 text-center text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                <input aria-label="Select exportSales on this page" checked={pageSelected} className="size-4 accent-primary disabled:cursor-not-allowed disabled:opacity-40" disabled={pageSelectableCount === 0} onChange={(event) => onTogglePageSelection(event.target.checked)} type="checkbox" />
              </th>
              { ["ExportSale", ...(visibleColumns.customer ? ["Customer"] : []), ...(visibleColumns.issuedOn ? ["Date"] : []), ...(visibleColumns.items ? ["Items"] : []), ...(visibleColumns.taxable ? ["Taxable"] : []), ...(visibleColumns.gst ? ["GST"] : []), ...(visibleColumns.total ? ["Total"] : []), ...(visibleColumns.status ? ["Status"] : []), ...(visibleColumns.invoice ? ["Invoice"] : []), ...(visibleColumns.action ? ["Action"] : [])].map((heading) => (
          ...
          <tbody>
            {entries.map((exportSale, index) => (
              <tr key={exportSale.id} className="border-b border-border/70 transition-colors last:border-b-0 hover:bg-muted/20">
                <td className="px-4 py-2.5 text-center">
                  <input aria-label={`Select ${exportSale.invoiceNumber}`} checked={selectedExportSaleIds.has(exportSale.id)} className="size-4 accent-primary disabled:cursor-not-allowed disabled:opacity-40" disabled={!canGenerateInvoiceFromExportSale(exportSale)} onChange={(event) => onToggleSelection(exportSale, event.target.checked)} title={exportSale.generatedSalesInvoiceNo ? `Already invoiced by ${exportSale.generatedSalesInvoiceNo}` : undefined} type="checkbox" />
                </td>
                <td className="px-4 py-2.5">
                  <button className="font-semibold text-foreground underline-offset-4 hover:underline" onClick={() => onView(exportSale)} title="View exportSale" type="button">{exportSale.invoiceNumber}</button>
                </td>
                {visibleColumns.customer ? <td className="px-4 py-2.5">
                  <button className={cn("font-medium underline-offset-4", exportSale.status === "draft" ? "hover:underline" : "cursor-not-allowed text-muted-foreground")} disabled={exportSale.status !== "draft"} onClick={() => onEdit(exportSale)} title={exportSale.status === "draft" ? "Edit exportSale" : "Submitted exportSales cannot be edited"} type="button">{exportSale.customerName}</button>
                </td> : null}
                {visibleColumns.issuedOn ? <td className="px-4 py-2.5">{formatDate(exportSale.issuedOn)}</td> : null}
                {visibleColumns.items ? <td className="px-4 py-2.5">{totalExportSaleQuantity(exportSale)}</td> : null}
                {visibleColumns.taxable ? <td className="px-4 py-2.5">{formatMoney(exportSale.subtotal)}</td> : null}
                {visibleColumns.gst ? <td className="px-4 py-2.5">{formatMoney(exportSale.taxAmount)}</td> : null}
                {visibleColumns.total ? <td className="px-4 py-2.5 font-semibold">{formatMoney(exportSale.amount)}</td> : null}
                {visibleColumns.status ? <td className="px-4 py-2.5"><StatusPill exportSale={exportSale} /></td> : null}
                {visibleColumns.invoice ? <td className="px-4 py-2.5 font-semibold text-sky-700">{exportSale.generatedSalesInvoiceNo || "-"}</td> : null}
                {visibleColumns.action ? <td className="px-4 py-2.5">
                  <WorkspaceRowActions
                    actions={[
                      ...(exportSale.status === "draft" ? [{ id: "confirm", label: "Confirm", icon: <Eye className="size-4" />, onSelect: () => onSetStatus(exportSale, "confirmed") }] : []),
                      ...(canAdminRevoke && exportSale.status === "confirmed" && !exportSale.generatedSalesInvoiceNo ? [{ id: "revoke", label: "Revoke by admin", icon: <RotateCcw className="size-4" />, onSelect: () => onRevoke(exportSale) }] : []),
                      ...(exportSale.status !== "cancelled" && !exportSale.generatedSalesInvoiceNo ? [{ id: "suspend", label: "Suspend", icon: <Trash2 className="size-4" />, tone: "destructive" as const, onSelect: () => onSetStatus(exportSale, "cancelled") }] : []),
                      ...(exportSale.status === "draft" ? [{ id: "force-delete", label: "Force delete", icon: <Trash2 className="size-4" />, tone: "destructive" as const, onSelect: () => onForceDelete(exportSale) }] : []),
                    ]}
                    {...(exportSale.status === "draft" ? { onEdit: () => onEdit(exportSale) } : {})}
      ...
      {entries.length === 0 ? <WorkspaceTableEmptyState>{loading ? "Loading exportSales..." : "No exportSales found."}</WorkspaceTableEmptyState> : null}
    </WorkspaceTablePanel>
  );
}

export function ExportSaleUpsertPage({ canAdminRevoke, errorMessage, loading, numbering, onBack, onRevoke, onSubmit, exportSale, settings }: { canAdminRevoke: boolean; errorMessage: string; loading: boolean; numbering: BillingDocumentNumberSettings; onBack: () => void; onRevoke?: () => void; onSubmit: (payload: ExportSaleSavePayload, printAfter?: boolean) => void; exportSale: ExportSale | null; settings: BillingDocumentLayoutSettings }) {
  const [activeTab, setActiveTab] = useState("details");
  const [workflowAction, setWorkflowAction] = useState<"draft" | "submit" | "revoke">(exportSale?.status === "confirmed" ? "revoke" : "draft");
  const [form, setForm] = useState<ExportSaleSavePayload>(() => exportSale ? exportSaleToPayload(exportSale) : {
  ...
  useEffect(() => {
    ...
    setForm((current) => current.invoiceNumber === nextExportSaleNumber ? current : { ...current, invoiceNumber: nextExportSaleNumber });
  }, [numbering, exportSale]);
  const [itemDraft, setItemDraft] = useState(() => createEmptyExportSale().items[0] ?? {
    colour: "",
    dcNo: "",
    description: "",
    hsnCode: "",
  ...
  const [editingContact, setEditingContact] = useState<ExportSaleLookupOption["record"] | null>(null);
  ...
  const [roundOffManual, setRoundOffManual] = useState(Boolean(exportSale && Number(exportSale.roundOff || 0) !== 0));
  const [billingAddressDraft, setBillingAddressDraft] = useState<ExportSaleAddressDraft>(() => exportSaleAddressDraftFromText(form.billingAddress, "Billing"));
  const [shippingAddressDraft, setShippingAddressDraft] = useState<ExportSaleAddressDraft>(() => exportSaleAddressDraftFromText(form.shippingAddress, "Shipping"));
  const [billingAddressChoice, setBillingAddressChoice] = useState("");
  const [shippingAddressChoice, setShippingAddressChoice] = useState("");
  const contactsQuery = useQuery({ queryFn: listExportSaleContacts, queryKey: ["billing", "exportSale", "lookups", "contacts"] });
  const workOrdersQuery = useQuery({ queryFn: listExportSaleWorkOrders, queryKey: ["billing", "exportSale", "lookups", "work-orders"] });
  const productsQuery = useQuery({ queryFn: listExportSaleProducts, queryKey: ["billing", "exportSale", "lookups", "products"] });
  const coloursQuery = useQuery({ queryFn: listExportSaleColours, queryKey: ["billing", "exportSale", "lookups", "colours"] });
  const sizesQuery = useQuery({ queryFn: listExportSaleSizes, queryKey: ["billing", "exportSale", "lookups", "sizes"] });
  const contactSaveMutation = useMutation({
    mutationFn: ({ id, payload }: { id?: string; payload: ExportSaleContactSavePayload }) => id ? updateExportSaleContact(id, payload) : createExportSaleContact(payload),
  });
  const masterSaveMutation = useMutation({
    mutationFn: ({ id, kind, payload }: { id?: string; kind: "products" | "workOrders"; payload: ExportSaleMasterSavePayload }) =>
      id ? updateExportSaleLookup(kind, id, masterPayload(kind, payload)) : createExportSaleLookup(kind, masterPayload(kind, payload)),
  });
  ...
  const contactAddressChoices = useMemo(() => buildExportSaleAddressChoices(selectedContact?.record), [selectedContact?.record]);
  const itemTotals = useMemo(() => computeExportSaleTotals(form.items, form.taxType), [form.items, form.taxType]);
  ...
  function applyAddressDraft(kind: "billing" | "shipping", draft: ExportSaleAddressDraft, choiceValue = "") {
  ...
  const tabs: WorkspaceAnimatedTab[] = [
    {
      ...
      content: (
        <div className="space-y-0">
          ...
          <div className="space-y-5">
            <Field label="Customer name" required>
              <WorkspaceLookup
                ...
                onValueChange={(value, option) => applyContactSelection(value, option as ExportSaleLookupOption | null | undefined)}
          ...
          <div className="space-y-5">
            <Field label="ExportSale number"><Input value={form.invoiceNumber} onChange={(event) => patch({ invoiceNumber: event.target.value.toUpperCase() })} /></Field>
            <Field label="Date"><WorkspaceDatePicker value={form.issuedOn} onValueChange={(value) => patch({ issuedOn: value })} /></Field>
            <Field label="ExportSale tax type">
              <WorkspaceSelect
                value={form.taxType}
  ...
  return (
    <WorkspacePage
      ...
      title={exportSale ? "Edit ExportSale" : "New ExportSale"}
      description="Create or update a tenant-isolated exportSale voucher."
    ...
    >
      <WorkspaceFormSurface>
          ...
          <div className="ml-auto flex items-center gap-2">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  aria-label="ExportSale workflow action"
                  className={cn(
                  ...
                  disabled={loading}
                  title="ExportSale workflow action"
                  type="button"
                  variant="outline"
                >
              ...
              <DropdownMenuContent align="end" className="w-20 min-w-20 rounded-md p-1">
                ...
                {(workflowAction === "submit" || exportSale?.status === "confirmed") && !exportSale?.generatedSalesInvoiceNo ? <><DropdownMenuSeparator /><DropdownMenuItem className="gap-1 px-2 text-xs" onSelect={() => setWorkflowAction("revoke")}><RotateCcw className="size-4" />Revoke</DropdownMenuItem></> : null}
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
        </WorkspaceFormActions>
            ...
            {editingContact ? (
              <ExportSaleContactQuickForm
                ...
                onSave={async (payload) => {
                  ...
                  toast.success("Contact saved", { description: exportSaleContactOption(saved).label });
                }}
                title="Edit contact"
              />
            ) : null}
        ...
        <Dialog open={Boolean(editingProduct)} onOpenChange={(open) => !open && setEditingProduct(null)}>
          <DialogContent className="rounded-md p-0 sm:max-w-3xl" onInteractOutside={(event) => event.preventDefault()}>
            {editingProduct ? <ExportSaleProductQuickForm initialValue={masterDraftFromRecord(editingProduct)} loading={masterSaveMutation.isPending} onCancel={() => setEditingProduct(null)} onSave={async (payload) => { const saved = await masterSaveMutation.mutateAsync({ id: editingProduct.id, kind: "products", payload }); await productsQuery.refetch(); patchDraft({ productName: exportSaleProductOption(saved).label }); setEditingProduct(null); toast.success("Product saved", { description: exportSaleProductOption(saved).label }); }} title="Edit product" /> : null}
          </DialogContent>
        </Dialog>
        <Dialog open={Boolean(editingWorkOrder)} onOpenChange={(open) => !open && setEditingWorkOrder(null)}>
          <DialogContent className="rounded-md p-0 sm:max-w-3xl" onInteractOutside={(event) => event.preventDefault()}>
            {editingWorkOrder ? <ExportSaleMasterQuickForm kind="workOrders" initialValue={masterDraftFromRecord(editingWorkOrder)} loading={masterSaveMutation.isPending} onCancel={() => setEditingWorkOrder(null)} onSave={async (payload) => { const saved = await masterSaveMutation.mutateAsync({ id: editingWorkOrder.id, kind: "workOrders", payload }); await workOrdersQuery.refetch(); patch({ workOrderNo: exportSaleWorkOrderOption(saved).value }); setEditingWorkOrder(null); toast.success("Work order saved", { description: exportSaleWorkOrderOption(saved).label }); }} title="Edit work order" /> : null}
          </DialogContent>
        </Dialog>
...
function ExportSaleContactQuickForm({ initialValue, loading, onCancel, onSave, title }: { initialValue: ExportSaleContactSavePayload; loading: boolean; onCancel: () => void; onSave: (payload: ExportSaleContactSavePayload) => Promise<void>; title: string }) {
  ...
  const addressTypesQuery = useQuery({ queryFn: listExportSaleAddressTypes, queryKey: ["billing", "exportSale", "lookups", "address-types"] });
  const countriesQuery = useQuery({ queryFn: () => listExportSaleLocations("countries"), queryKey: ["billing", "exportSale", "lookups", "countries"] });
  const statesQuery = useQuery({ queryFn: () => listExportSaleLocations("states"), queryKey: ["billing", "exportSale", "lookups", "states"] });
  const districtsQuery = useQuery({ queryFn: () => listExportSaleLocations("districts"), queryKey: ["billing", "exportSale", "lookups", "districts"] });
  const citiesQuery = useQuery({ queryFn: () => listExportSaleLocations("cities"), queryKey: ["billing", "exportSale", "lookups", "cities"] });
  const pincodesQuery = useQuery({ queryFn: () => listExportSaleLocations("pincodes"), queryKey: ["billing", "exportSale", "lookups", "pincodes"] });
        ...
        <div className="grid gap-4">
          <label className="grid gap-2">
            ...
            <WorkspaceLookup
              ...
              options={(addressTypesQuery.data ?? []).filter((record) => record.isActive !== false).map(exportSaleContactOption)}
...
function ContactLocationLookup({ kind, label, loading, onCreate, onPick, options, value }: { kind: ExportSaleLocationKind; label: string; loading: boolean; onCreate: (kind: ExportSaleLocationKind, name: string) => Promise<ExportSaleLookupOption | undefined>; onPick: (record: ExportSaleLocationRecord) => void; options: ExportSaleLocationRecord[]; value: string }) {
  const lookupOptions = options.filter((record) => record.status !== "inactive").map(exportSaleLocationOption);
  return (
    <label className="grid gap-2">
      <Label>{label}</Label>
      <WorkspaceLookup
        ...
        onValueChange={(selected, option) => {
          const record = ((option as ExportSaleLookupOption | undefined)?.record as ExportSaleLocationRecord | undefined) ?? options.find((item) => item.id === selected);
...
function locationPayload(kind: ExportSaleLocationKind, name: string, form: ExportSaleContactSavePayload) {
...
function locationPatch(kind: ExportSaleLocationKind, record: ExportSaleLocationRecord, form: ExportSaleContactSavePayload): ExportSaleContactSavePayload {
...
function contactDraftFromRecord(record?: ExportSaleLookupRecord, initialName = ""): ExportSaleContactSavePayload {
...
function ExportSaleMasterQuickForm({ initialValue, kind, loading, onCancel, onSave, title }: { initialValue: ExportSaleMasterSavePayload; kind: "products" | "workOrders"; loading: boolean; onCancel: () => void; onSave: (payload: ExportSaleMasterSavePayload) => Promise<void>; title: string }) {
  const [form, setForm] = useState(initialValue);
...
function masterDraftFromRecord(record?: ExportSaleLookupRecord, initialName = ""): ExportSaleMasterSavePayload {
  return {
    code: record?.code ?? "",
...
function ExportSaleItemsSection({
  ...
  renderProductCreateForm: (context: { initialName: string; onCancel: () => void; onCreated: (option: ExportSaleLookupOption) => void }) => ReactNode;
  ...
  return (
    <div className="mt-8 px-0 pb-0 pt-5">
      <div>
        <h3 className="text-lg font-semibold tracking-normal text-foreground underline decoration-foreground/70 underline-offset-4">ExportSale Items</h3>
            ...
            <div ref={rowRef} className="grid gap-1" style={{ gridTemplateColumns: templateColumns }}>
              ...
              <Field label="Product name">
                <WorkspaceLookup
                  ...
                  onValueChange={(value, option) => onProductSelect(value, option as ExportSaleLookupOption | null | undefined)}
              ...
              {!items.length ? (
                <tr>
                  <td className="px-3 py-6 text-center text-sm text-muted-foreground" colSpan={11 + (showPo ? 1 : 0) + (showDc ? 1 : 0) + (showColour ? 1 : 0) + (showSize ? 1 : 0) + (splitTax ? 2 : 1)}>
                    Add exportSale items to see them here.
                  </td>
                </tr>
              ) : null}
            </tbody>
...
function StatusPill({ exportSale, status }: { exportSale?: ExportSale; status?: ExportSale["status"] }) {
  const label = exportSale?.generatedSalesInvoiceNo ? "invoiced" : status ?? exportSale?.status ?? "draft";
...
function ExportSaleProductQuickForm({ initialValue, loading, onCancel, onSave, title }: { initialValue: ExportSaleMasterSavePayload; loading: boolean; onCancel: () => void; onSave: (payload: ExportSaleMasterSavePayload) => Promise<void>; title: string }) {
  const [form, setForm] = useState(initialValue);
  const categoriesQuery = useQuery({ queryFn: listExportSaleProductCategories, queryKey: ["billing", "exportSale", "lookups", "product-categories"] });
  const hsnCodesQuery = useQuery({ queryFn: listExportSaleHsnCodes, queryKey: ["billing", "exportSale", "lookups", "hsn-codes"] });
  const unitsQuery = useQuery({ queryFn: listExportSaleUnits, queryKey: ["billing", "exportSale", "lookups", "units"] });
  const taxesQuery = useQuery({ queryFn: listExportSaleTaxes, queryKey: ["billing", "exportSale", "lookups", "taxes"] });

  function patchProduct(next: Partial<ExportSaleMasterSavePayload>) { setForm((current) => ({ ...current, ...next })); }
  ...
  const hsnOptions = (hsnCodesQuery.data ?? []).map((record) => ({ ...exportSaleCommonOption(record), label: record.code || record.name || record.id, value: record.id }));
  const unitOptions = (unitsQuery.data ?? []).map(exportSaleCommonOption);
  const taxOptions = (taxesQuery.data ?? []).map((record) => ({ ...exportSaleCommonOption(record), label: record.name || record.code || `${record.ratePercent ?? record.taxRate ?? 0}%`, value: record.id }));
...
function ProductPopupLookup({ label, loading, numericOnly = false, onCreate, onValueChange, options, placeholder, value }: { label: string; loading: boolean; numericOnly?: boolean; onCreate: (name: string) => Promise<ExportSaleLookupRecord>; onValueChange: (value: string, option?: ExportSaleLookupOption | null) => void; options: ExportSaleLookupOption[]; placeholder: string; value: string }) {
  const sanitize = numericOnly ? (input: string) => input.replace(/[^0-9.]/g, "").replace(/(\..*)\./g, "$1") : undefined;
  return <label className="grid gap-2"><Label>{label}</Label><WorkspaceLookup createLabel={`Create ${label.toLowerCase()}`} createMode="inline" emptyLabel={`No ${label.toLowerCase()} found. Type a value to create it.`} loading={loading} options={options} placeholder={placeholder} value={value} {...(sanitize ? { sanitizeInput: sanitize } : {})} onCreate={async (name) => exportSaleCommonOption(await onCreate(sanitize ? sanitize(name) : name))} onValueChange={onValueChange} /></label>;
}

function canGenerateInvoiceFromExportSale(exportSale: ExportSale) {
  return exportSale.status !== "cancelled" && !exportSale.generatedSalesInvoiceNo;
}
...
function computeExportSaleLine(item: ExportSaleSavePayload["items"][number], taxType: ExportSaleTaxType) {
...
function computeExportSaleTotals(items: ExportSaleSavePayload["items"], taxType: ExportSaleTaxType) {
  return items.reduce(
    (totals, item) => {
      const line = computeExportSaleLine(item, taxType);
      return {
