...
export function ExportSaleWorkspace() {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: async (exportSale) => {
      ...
      toast.success(
        view.mode === "upsert" && view.exportSale ? "ExportSale updated" : "ExportSale created",
        {
    ...
    onError: (error) => {
      toast.error("ExportSale save failed", {
  ...
  const statusMutation = useMutation({
    ...
    onSuccess: async (exportSale) => {
      ...
      toast.success("ExportSale status updated", {
  ...
  const revokeMutation = useMutation({
    ...
    onSuccess: async (exportSale) => {
      await queryClient.invalidateQueries({ queryKey: ["billing", "exportSales"] });
      toast.success("ExportSale revoked", {
        description: `${exportSale.invoiceNumber} is editable again.`
      });
    },
    onError: (error) => {
      toast.error("ExportSale revoke failed", {
        description: error instanceof Error ? error.message : "Please try again."
      });
    }
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error) => {
      toast.error("ExportSale could not be deleted", {
        description:
          error instanceof Error ? error.message : "Only draft exportSales can be deleted."
      });
    }
  });
        ...
        <WorkspaceTablePanel>
          <WorkspaceTableEmptyState>
            {exportSalesQuery.error instanceof Error
              ...
              : "ExportSales could not be loaded."}
          </WorkspaceTableEmptyState>
        </WorkspaceTablePanel>
      ) : null}
      <ExportSaleList
...
function ExportSaleList({
  ...
  return (
    <WorkspaceTablePanel>
          ...
          <thead className="bg-muted/50">
            <tr>
              <th className="w-12 border-b border-border/70 px-4 py-3.5 text-center text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                <input
                  aria-label="Select exportSales on this page"
              ...
              >
                <td className="px-4 py-2.5 text-center">
                  <input
                    ...
                    className="size-4 accent-primary disabled:cursor-not-allowed disabled:opacity-40"
                    disabled={!canGenerateInvoiceFromExportSale(exportSale)}
                    onChange={(event) => onToggleSelection(exportSale, event.target.checked)}
                    title={
                      exportSale.generatedSalesInvoiceNo
                        ? `Already invoiced by ${exportSale.generatedSalesInvoiceNo}`
                        : undefined
                    }
                    type="checkbox"
                  />
                  ...
                  <td className="px-4 py-2.5">
                    <button
                      ...
                      title={
                        exportSale.status === "draft"
                          ? "Edit exportSale"
                          : "Submitted exportSales cannot be edited"
                      }
                      type="button"
                    >
                      {exportSale.customerName}
                    ...
                    <WorkspaceRowActions
                      actions={[
                        ...
                        !exportSale.generatedSalesInvoiceNo
                          ...
                          : []),
                        ...(exportSale.status !== "cancelled" && !exportSale.generatedSalesInvoiceNo
                          ? [
                              {
                                id: "suspend",
                                label: "Suspend",
      ...
      {entries.length === 0 ? (
        <WorkspaceTableEmptyState>
          {loading ? "Loading exportSales..." : "No exportSales found."}
        </WorkspaceTableEmptyState>
      ) : null}
    </WorkspaceTablePanel>
  );
...
export function ExportSaleUpsertPage({
...
function StatusPill({
  ...
  const label = exportSale?.generatedSalesInvoiceNo
    ? "invoiced"
    : (status ?? exportSale?.status ?? "draft");
  const tone =
    label === "invoiced"
      ? "info"
      : label === "confirmed"
