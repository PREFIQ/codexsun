...
export const exportSaleLineItemSchema = z.object({
  colour: z.string(),
  dcNo: z.string(),
...
export const exportSalesSchema = z.object({
  ...
  items: z.array(exportSaleLineItemSchema).min(1, "Add at least one line item."),
  notes: z.string(),
  roundOff: z.number(),
  shippingAddress: z.string().trim().min(1, "Shipping address is required."),
  status: z.enum(["draft", "confirmed", "cancelled"]),
  taxType: z.string().trim().min(1, "Export sales tax type is required."),
