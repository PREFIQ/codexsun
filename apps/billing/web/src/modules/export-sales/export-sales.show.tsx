...
type ExportSaleEntryToolId = "assign" | "attachments" | "downloadPdf" | "email" | "tags" | "whatsapp";
...
export function ExportSaleShowPage({
  ...
  const activityItems = useMemo(
    () => [
      ...toolActivities,
      { createdAt: exportSale.updatedAt, id: "updated", message: `ExportSale updated${exportSale.generatedSalesInvoiceNo ? ` and linked to ${exportSale.generatedSalesInvoiceNo}` : ""}` },
  ...
  function recordActivity(message: string) {
    setToolActivities((current) => [{ createdAt: new Date().toISOString(), id: `${Date.now()}-${current.length}`, message }, ...current]);
  }

  function addComment() {
    const body = comment.trim();
    if (!body) return;
    setComments((current) => [{ body, createdAt: new Date().toISOString(), id: `${Date.now()}-${current.length}` }, ...current]);
    recordActivity("Added a comment");
    setComment("");
  }

  function addListValue(value: string, setValue: (value: string) => void, setValues: React.Dispatch<React.SetStateAction<string[]>>, message: (value: string) => string) {
  ...
  function removeListValue(value: string, setValues: React.Dispatch<React.SetStateAction<string[]>>) {
        ...
        <div className="mb-4 grid gap-3 print:hidden">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex flex-wrap items-center justify-end gap-2">
              ...
              <Button disabled={!canEdit} title={canEdit ? "Edit exportSale" : "Submitted exportSales cannot be edited"} type="button" variant="outline" className="rounded-xl" onClick={onEdit}><Pencil className="size-4" />Edit</Button>
            ...
            <CardContent className="space-y-8">
              ...
              {comments.length || exportSale.notes ? (
                <div className="space-y-2">
                  {exportSale.notes ? <SideNote body={exportSale.notes} meta="Saved notes" title="System" /> : null}
                ...
                <div key={tool.id} className="border-b border-border/70 last:border-b-0">
                  ...
                  {openTool === tool.id ? (
                    <div className="px-3 pb-3">
                      {tool.id === "email" ? <InlineSend value={emailAddress} placeholder="Email address" onChange={setEmailAddress} onSend={() => { const value = emailAddress.trim(); if (!value) return; recordActivity(`Queued exportSale email to ${value}`); setEmailAddress(""); }} /> : null}
                      {tool.id === "assign" ? <Input value={assigneeInput} onChange={(event) => setAssigneeInput(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); addListValue(assigneeInput, setAssigneeInput, setAssignees, (value) => `Assigned exportSale to ${value}`); } }} placeholder="User name or email" className="h-9 rounded-md" /> : null}
...
function InlineSend({
  ...
  return (
    <div className="flex gap-2">
      <Input value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} className="h-9 rounded-md" />
      <Button disabled={!value.trim()} onClick={onSend} type="button" className="size-9 rounded-md p-0"><Send className="size-4" /></Button>
    </div>
  );
}
