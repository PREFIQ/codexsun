...
export function PaymentForm({
  ...
  useEffect(() => {
    if (value?.paymentNumber) return;
    void getBillingSettings().then((settings) =>
      setForm((current) =>
        current.paymentNumber
          ? current
          : { ...current, paymentNumber: formatDocumentNumber(settings.numbering.payment) }
      )
    );
  }, [value?.paymentNumber]);
  const patch = (key: keyof PaymentInput, next: string | number) =>
    setForm((current) => ({ ...current, [key]: next }));
  const details = (
    <WorkspaceFormGrid>
      <WorkspaceFormField label="Supplier name *">
        <Input
          required
      ...
      <WorkspaceFormField label="Mode of payment">
        <WorkspaceSelect
          ...
          options={modes}
          value={String(form.paymentMode ?? "cash")}
          onValueChange={(value) => patch("paymentMode", value)}
        />
      </WorkspaceFormField>
      <WorkspaceFormField
      ...
      >
        <WorkspaceSelect
          ...
          value={String(
            form.bankAccount || (form.paymentMode === "cash" ? "Cash" : "Primary bank account")
          )}
  ...
  const allocation = (
    <div className="space-y-4">
      <div className="rounded-md border border-dashed p-5 text-sm text-muted-foreground">
        Allocate this payment against purchase bills in the next step.
      </div>
      <WorkspaceFormGrid>
        <WorkspaceFormField label="Bill no">
          <Input placeholder="Select purchase bill" />
        </WorkspaceFormField>
  ...
  return (
    <WorkspacePage
      ...
      description="Create an outgoing payment with allocation details."
      onBack={onCancel}
    >
      <form
        onSubmit={(event) => {
