...
export function SalesPrintRoutePage() {
  ...
  return (
    <WorkspacePage
      title={sale ? `${sale.invoiceNumber} print` : "Sales print"}
      description="Printable invoice surface for the sales workspace."
      actions={
    ...
    >
      <PageTitle title={sale ? `${sale.invoiceNumber} Print` : "Sales Print"} />
      {sale ? <SalesPrintDocument sale={sale} /> : <div className="rounded-md border border-border/70 bg-card/95 px-4 py-8 text-sm text-muted-foreground">{saleQuery.isLoading ? "Loading sale print view..." : "Sale print record was not found."}</div>}
    </WorkspacePage>
  );
}

export function SalesPrintDocument({ sale }: { sale: Sale }) {
  return (
    <WorkspacePrintSheet>
      <article className="border border-slate-300 bg-white p-6 text-[10px] text-slate-900 shadow-sm">
        <header className="grid gap-4 border-b border-slate-300 pb-4 md:grid-cols-[1fr_auto]">
          <div>
            ...
            <p className="mt-2 max-w-xl text-sm leading-6 text-slate-600">
              Sales document prepared from the billing workspace with the same structured invoice rhythm used in the reference system.
            </p>
          </div>
        ...
        <footer className="mt-8 grid gap-4 border-t border-slate-300 pt-4 md:grid-cols-[1fr_16rem]">
          <div className="text-[9px] leading-5 text-slate-500">
            Generated from the CODEXSUN billing sales workspace. Review the record activity in the app before dispatching the invoice externally.
          </div>
