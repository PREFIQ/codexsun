...
import { paymentTermsDefinition } from "@codexsun/core-web/modules/common/others/payment-terms";
import { salesTypesDefinition } from "@codexsun/core-web/modules/common/others/sales-types";
import { coloursDefinition } from "@codexsun/core-web/modules/common/products/colours";
import { hsnCodesDefinition } from "@codexsun/core-web/modules/common/products/hsn-codes";
import { productTypesDefinition } from "@codexsun/core-web/modules/common/products/product-types";
import { sizesDefinition } from "@codexsun/core-web/modules/common/products/sizes";
import { taxesDefinition } from "@codexsun/core-web/modules/common/products/taxes";
import { unitsDefinition } from "@codexsun/core-web/modules/common/products/units";
import { transportsDefinition } from "@codexsun/core-web/modules/common/workorder/transports";
...
export function EntryDocumentWorkspace({ kind }: { kind: EntryKind }) {
  ...
  const title = entryKindTitle(kind);
  const plural = entryKindPlural(kind);
  ...
  return (
    <WorkspacePage
      ...
      description={
        kind === "quotation"
          ? "Create, review, and convert customer quotations."
          : kind === "purchase"
            ? "Create, review, and print supplier purchase bills."
            : kind === "exportSales"
              ? "Create, review, and print export sales invoices."
              : "Create, review, and print customer invoices."
      }
      actions={
    ...
    >
      <WorkspaceFilters
        searchPlaceholder={`Search ${title.toLowerCase()} number, ${entryPartyLabel(kind).toLowerCase()}, or reference`}
        searchValue={search}
          ...
          <table className="w-full min-w-[980px] border-collapse text-sm">
            <thead className="bg-muted/45">
              <tr>
                ...
                <WorkspaceTableHeaderCell>{entryPartyLabel(kind)}</WorkspaceTableHeaderCell>
...
function EntryShowPage({
  ...
  const details: Array<[string, ReactNode]> = [
    [entryNumberLabel(kind), entry.documentNo],
    ["Date", formatDate(entry.documentDate)],
    [entryPartyLabel(kind), entry.customerName],
  ...
  if (kind === "purchase") {
    details.splice(4, 0, ["Supplier bill no", entry.supplierBillNo || "-"], ["Supplier bill date", formatDate(entry.supplierBillDate)]);
  }
  ...
  return (
    <WorkspacePage
      title={`${entryKindTitle(kind)} ${entry.documentNo}`}
...
function EntryUpsertPageView({
  ...
  const [itemDraft, setItemDraft] = useState<EntryLineRecord>(() => emptyLine());
  ...
  function updateItemDraft(patch: Partial<EntryLineRecord>) {
  ...
  function saveDraftLine() {
    ...
    setFormError(null);
    setForm((current) => {
      const nextLine = { ...itemDraft, sortOrder: editingLineIndex === null ? current.lines.length + 1 : itemDraft.sortOrder || editingLineIndex + 1 };
      const nextLines = editingLineIndex === null
        ? [...current.lines, nextLine]
        : current.lines.map((line, index) => (index === editingLineIndex ? nextLine : line));
      return {
        ...current,
        lines: nextLines.map((line, index) => ({ ...line, sortOrder: index + 1 })),
      };
    });
    resetItemDraft();
  }

  function editDraftLine(index: number) {
    const selected = form.lines[index];
    if (!selected) return;
  ...
  const tabs: WorkspaceAnimatedTab[] = [
    {
      content: (
        <div className="grid gap-6 lg:grid-cols-2">
          <div className="space-y-4">
            <Field label={entryPartyFieldLabel(kind)} required>
              <WorkspaceLookup
                createDescription={`Create a ${entryPartyLabel(kind).toLowerCase()} contact without leaving the entry.`}
          ...
          <div className="space-y-4">
            <Field label={entryNumberFieldLabel(kind)} required>
              <Input value={form.documentNo} onChange={(event) => setForm((current) => ({ ...current, documentNo: event.target.value }))} />
            </Field>
            <Field label="Date" required>
              <WorkspaceDatePicker value={form.documentDate} onValueChange={(value) => setForm((current) => ({ ...current, documentDate: value }))} />
            </Field>
            <Field label={entryTaxTypeLabel(kind)}>
      ...
      content: (
        <div className="space-y-4">
            ...
            <div className="overflow-x-auto">
              <div className="min-w-[1120px]">
                <div
                  ...
                  style={{
                    gridTemplateColumns: [
                      ...(billingLayout.usePo ? ["minmax(6rem,0.7fr)"] : []),
                      ...(billingLayout.useDc ? ["minmax(6rem,0.7fr)"] : []),
                      "minmax(18rem,1.9fr)",
                      "minmax(16rem,1.2fr)",
                      ...(billingLayout.useColour ? ["minmax(7rem,0.8fr)"] : []),
                      ...(billingLayout.useSize ? ["minmax(7rem,0.8fr)"] : []),
                      "minmax(6rem,0.7fr)",
                      "minmax(7rem,0.7fr)",
                      "auto",
                    ].join(" "),
                ...
                >
                  {billingLayout.usePo ? (
                    <Field label="PO">
                      <Input value={itemDraft.poNo || ""} onChange={(event) => updateItemDraft({ poNo: event.target.value })} />
                    </Field>
                  ) : null}
                  {billingLayout.useDc ? (
                    <Field label="DC">
                      <Input value={itemDraft.dcNo || ""} onChange={(event) => updateItemDraft({ dcNo: event.target.value })} />
                    </Field>
                  ) : null}
                  <Field label="Product name" required>
                    <WorkspaceLookup
                      ...
                      trailingAction={
                        <LookupTrailingAction
                          canEdit={Boolean(itemDraft.productId)}
                          onEdit={() => {
                            const selected = products.find((item) => item.id === itemDraft.productId);
                            if (!selected) return;
                      ...
                      value={itemDraft.productId || itemDraft.productName}
                      onCreate={async (name) => {
                      ...
                      onValueChange={(value, option) => {
                        ...
                        if (!product) {
                          updateItemDraft({ productName: option?.label ?? value });
                          return;
                        }
                        updateItemDraft({
                  ...
                  <Field label="Description">
                    <Input value={itemDraft.description || ""} onChange={(event) => updateItemDraft({ description: event.target.value })} />
                  </Field>
                  {billingLayout.useColour ? (
                    <Field label="Colour">
                      <WorkspaceLookup
                        ...
                        value={itemDraft.colourId || itemDraft.colourName || ""}
                        onCreate={async (name) => {
                          const created = await createCommonMasterOption(coloursDefinition.path, { name });
                          await onRefreshSupport();
                          return commonOption(created, "name");
                        }}
                        onValueChange={(value, option) => updateItemDraft({ colourId: value, colourName: option?.label ?? value })}
                      />
                    </Field>
                  ) : null}
                  {billingLayout.useSize ? (
                    <Field label="Size">
                      <WorkspaceLookup
                        ...
                        value={itemDraft.sizeId || itemDraft.sizeName || ""}
                        onCreate={async (name) => {
                          const created = await createCommonMasterOption(sizesDefinition.path, { name });
                          await onRefreshSupport();
                          return commonOption(created, "name");
                        }}
                        onValueChange={(value, option) => updateItemDraft({ sizeId: value, sizeName: option?.label ?? value })}
                      />
                    </Field>
                  ) : null}
                  <Field label="Quantity">
                    <Input type="number" value={String(itemDraft.quantity)} onChange={(event) => updateItemDraft({ quantity: Number(event.target.value || 0) })} />
                  </Field>
                  <Field label="Price">
                    <Input type="number" value={String(itemDraft.rate)} onChange={(event) => updateItemDraft({ rate: Number(event.target.value || 0) })} />
                  </Field>
                  <div className="flex items-end gap-2">
                    <Button type="button" className="h-11 rounded-md px-4" onClick={saveDraftLine}>
                      <Plus className="size-4" />
                      {editingLineIndex === null ? "Add" : "Update"}
                    </Button>
                    {editingLineIndex !== null ? (
                      <Button type="button" variant="outline" className="h-11 rounded-md px-4" onClick={resetItemDraft}>
                        <X className="size-4" />
                        Cancel
                      </Button>
                    ) : null}
          ...
          <WorkspaceLineTable
            ...
            data={form.lines}
            minWidth="1700px"
            {...(kind === "quotation" ? { onEdit: (_row: EntryLineRecord, index: number) => editDraftLine(index) } : {})}
            rowKey={(row) => row.id}
            onAdd={() => setForm((current) => ({ ...current, lines: [...current.lines, emptyLine()] }))}
            onDelete={(_, index) => {
              setForm((current) => ({
                ...current,
                lines: current.lines.filter((__, itemIndex) => itemIndex !== index).map((line, lineIndex) => ({ ...line, sortOrder: lineIndex + 1 })),
              }));
              if (editingLineIndex === index) resetItemDraft();
            }}
          />
        </div>
      ),
      ...
      content: (
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Field label="E-way bill date">
            <WorkspaceDatePicker value={form.ewayBillDate} onValueChange={(value) => setForm((current) => ({ ...current, ewayBillDate: value }))} />
          </Field>
      ...
      content: (
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <Field label="Ack date">
            <WorkspaceDatePicker value={form.ackDate} onValueChange={(value) => setForm((current) => ({ ...current, ackDate: value }))} />
          </Field>
  ...
  const visibleTabs = kind === "quotation"
          ...
          content: (
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <Field label="Due date">
                <WorkspaceDatePicker value={form.dueDate} onValueChange={(value) => setForm((current) => ({ ...current, dueDate: value }))} />
              </Field>
    ...
    : [
              ...
              {kind === "purchase" ? (
                <>
                  <Field label="Supplier bill no">
                    <Input value={form.supplierBillNo} onChange={(event) => setForm((current) => ({ ...current, supplierBillNo: event.target.value }))} />
                  </Field>
                  <Field label="Supplier bill date">
                    <WorkspaceDatePicker value={form.supplierBillDate} onValueChange={(value) => setForm((current) => ({ ...current, supplierBillDate: value }))} />
                  </Field>
                </>
              ) : (
        ...
        ...tabs.filter((tab) => !["general", "items"].includes(tab.value)),
      ];
  ...
  return (
    <WorkspaceUpsertPage
      ...
      description={`Enter the ${entryKindTitle(kind).toLowerCase()} details and save without leaving the list.`}
      title={entry ? `Edit ${entry.documentNo}` : `New ${entryKindTitle(kind).toLowerCase()}`}
    >
      <div className="space-y-4">
        ...
        <WorkspaceFormPanel
          ...
          {...(kind !== "quotation" ? { title: `${entryKindTitle(kind)} entry` } : {})}
        >
          <WorkspaceAnimatedTabs
            tabs={visibleTabs}
            value={activeTab}
            ...
            {...(kind === "quotation" ? {
              contentClassName: "mt-0 px-5 pb-5 pt-3",
          ...
          <div className={cn("mt-6 flex justify-end", kind === "quotation" && "hidden")}>
            <Button type="button" onClick={submit}>
              <Save className="size-4" />
              Save {entryKindTitle(kind).toLowerCase()}
            </Button>
          </div>
        </WorkspaceFormPanel>
      </div>
  ...
  return (
    <WorkspacePrintSheet>
      <div className="border-b border-black px-4 py-4 text-center">
        <div className="text-lg font-semibold tracking-wide">{entryPrintTitle(entry.kind)}</div>
      ...
      <div className="grid grid-cols-2 gap-6 px-4 py-4">
        <div>
          <div className="text-[11px] font-semibold uppercase">{entry.kind === "purchase" ? "Supplier" : "Bill To"}</div>
        ...
        <div className="text-right">
          ...
          <div className="mt-1 text-xs">{entry.kind === "purchase" ? `Supplier bill: ${entry.supplierBillNo || "-"}` : `Due: ${formatDate(entry.dueDate)}`}</div>
        </div>
      </div>
...
function formFromEntry(entry: EntryRecord | null, _kind: EntryKind): EntryFormState {
  return {
    ...
    supplierBillDate: entry?.supplierBillDate ?? "",
...
function payloadFromForm(form: EntryFormState) {
  return {
    ...
    source: form.source,
    status: form.status,
    supplierBillDate: form.supplierBillDate || null,
...
function validateForm(form: EntryFormState, kind: EntryKind) {
  const errors: string[] = [];
  if (!form.documentDate) errors.push("Date is required.");
  if (!form.customerName.trim()) errors.push(`${entryPartyLabel(kind)} is required.`);
  return errors;
}
...
function entryKindTitle(kind: EntryKind) {
...
function entryKindPlural(kind: EntryKind) {
...
function entryPartyLabel(kind: EntryKind) {
  return kind === "purchase" ? "Supplier" : "Customer";
}

function entryPartyFieldLabel(kind: EntryKind) {
  return `${entryPartyLabel(kind)} name`;
}

function entryNumberLabel(kind: EntryKind) {
  if (kind === "quotation") return "Quotation no";
  if (kind === "sales") return "Sales no";
  if (kind === "purchase") return "Purchase no";
  return "Export sales no";
}

function entryNumberFieldLabel(kind: EntryKind) {
  if (kind === "quotation") return "Quotation number";
  if (kind === "sales") return "Sales number";
  if (kind === "purchase") return "Purchase number";
  return "Export sales number";
}

function entryTaxTypeLabel(kind: EntryKind) {
  if (kind === "quotation") return "Quotation tax type";
  if (kind === "sales") return "Sales tax type";
  if (kind === "purchase") return "Purchase tax type";
  return "Export sales tax type";
}

function entryPrintTitle(kind: EntryKind) {
  if (kind === "quotation") return "QUOTATION";
  if (kind === "purchase") return "PURCHASE";
