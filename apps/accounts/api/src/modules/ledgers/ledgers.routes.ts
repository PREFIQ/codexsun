...
export async function registerLedgersRoutes(app: FastifyInstance) {
  app.get("/accounts/groups", async (request) => ok(await service.groups(databaseName(request.headers["x-tenant-db"])), { requestId: request.id }));
  ...
  app.get("/accounts/ledgers/lookup", async (request) => ok(await service.lookup(databaseName(request.headers["x-tenant-db"])), { requestId: request.id }));
  app.get("/accounts/ledgers/:id", async (request, reply) => {
    ...
    if (!ledger) return reply.code(404).send({ error: { code: "LEDGER_NOT_FOUND", message: "Ledger was not found." }, success: false });
    return ok(ledger, { requestId: request.id });
  });
  app.post("/accounts/ledgers", async (request) => ok(await service.create(databaseName(request.headers["x-tenant-db"]), request.body as LedgerSavePayload), { requestId: request.id }));
  app.put("/accounts/ledgers/:id", async (request, reply) => {
    const { id } = request.params as { id: string };
    const ledger = await service.update(databaseName(request.headers["x-tenant-db"]), id, request.body as LedgerSavePayload);
    if (!ledger) return reply.code(404).send({ error: { code: "LEDGER_NOT_FOUND", message: "Ledger was not found." }, success: false });
    return ok(ledger, { requestId: request.id });
  });
}
