...
export async function registerVouchersRoutes(app: FastifyInstance) {
  ...
  app.get("/accounts/vouchers/:id", async (request, reply) => {
    ...
    if (!voucher)
      return reply
        .code(404)
        .send({
          error: { code: "VOUCHER_NOT_FOUND", message: "Voucher was not found." },
          success: false
        });
    return ok(voucher, { requestId: request.id });
  });
  app.post("/accounts/vouchers", async (request) =>
    ok(
      await service.create(
        databaseName(request.headers["x-tenant-db"]),
        request.body as VoucherSavePayload
      ),
      { requestId: request.id }
    )
  );
  ...
  app.post("/accounts/postings/billing", async (request) =>
    ok(
      await service.postSource(
        databaseName(request.headers["x-tenant-db"]),
        request.body as AccountsPostingRequest
      ),
      { requestId: request.id }
    )
  );
