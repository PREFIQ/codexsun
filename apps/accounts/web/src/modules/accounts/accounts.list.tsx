...
export function LedgersList({
  ...
  return (
    <WorkspaceTablePanel>
            ...
            {ledgers.map((ledger) => (
              <tr className="border-b border-border/70 last:border-0" key={ledger.id}>
                ...
                <td className="px-4 py-2.5 text-right tabular-nums">
                  {money(ledger.currentDebit)}
                </td>
                <td className="px-4 py-2.5 text-right tabular-nums">
                  {money(ledger.currentCredit)}
                </td>
                ...
                <td className="px-4 py-2.5">
                  <WorkspaceStatusBadge
                    label={ledger.status}
                    tone={ledger.status === "active" ? "success" : "neutral"}
                  />
                </td>
                <td className="px-4 py-2.5 text-right">
                  <Button type="button" variant="outline" size="sm" onClick={() => onEdit(ledger)}>
      ...
      {!ledgers.length && !loading ? (
        <WorkspaceTableEmptyState>No ledgers found.</WorkspaceTableEmptyState>
      ) : null}
    </WorkspaceTablePanel>
  );
}
  ...
  return (
    <WorkspaceTablePanel>
            ...
            {vouchers.map((voucher) => (
              <tr className="border-b border-border/70 last:border-0" key={voucher.id}>
                ...
                <td className="px-4 py-2.5 text-muted-foreground">
                  {voucher.sourceDocumentNo ?? voucher.sourceApp ?? "-"}
                </td>
                ...
                <td className="px-4 py-2.5">
                  <WorkspaceStatusBadge
                    label={voucher.tallySyncStatus}
                    tone={
                      voucher.tallySyncStatus === "synced"
                        ? "success"
                        : voucher.tallySyncStatus === "failed"
                          ? "danger"
                          : "warning"
                    }
                  />
                  ...
                  <WorkspaceStatusBadge
                    ...
                    tone={
                      voucher.status === "posted"
                        ? "success"
                        : voucher.status === "cancelled" || voucher.status === "reversed"
                          ? "danger"
                          : "warning"
                    }
                  />
      ...
      {!vouchers.length && !loading ? (
        <WorkspaceTableEmptyState>
          No vouchers found. Billing postings and manual vouchers will appear here.
        </WorkspaceTableEmptyState>
      ) : null}
    </WorkspaceTablePanel>
  );
