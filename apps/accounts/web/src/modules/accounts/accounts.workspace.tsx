...
export function AccountsWorkspace({
  ...
  page?: "overview" | "ledgers" | "vouchers" | "reports" | "settings";
}) {
  const initialMode =
    page === "ledgers"
      ? "ledgers"
  ...
  const ledgerMutation = useMutation({
    mutationFn: ({ ledger, payload }: { ledger: Ledger | null; payload: LedgerSavePayload }) =>
      ledger ? updateLedger(ledger.id, payload) : createLedger(payload),
    onSuccess: async () => {
  ...
  if (view.mode === "ledgers") {
    ...
    return (
      <WorkspacePage
        title="Ledgers"
        description="Accounts ledgers with group, classification, balance, and Tally-ready names."
        actions={
          <NewButton
            label="New ledger"
            onClick={() => setView({ mode: "ledger-upsert", ledger: null })}
          />
        }
      >
        <WorkspaceFilters
          searchPlaceholder="Search ledger, code, group, or classification"
          searchValue={search}
        ...
        />
        <LedgersList
          ledgers={pageRows(ledgers, currentPage, rowsPerPage)}
          loading={ledgersQuery.isLoading}
          onEdit={(ledger) => setView({ mode: "ledger-upsert", ledger })}
        />
        <WorkspacePagination
          page={currentPage}
          rowsPerPage={rowsPerPage}
          showingLabel={buildShowingLabel(currentPage, rowsPerPage, ledgers.length)}
          singularLabel="ledger"
          totalCount={ledgers.length}
          totalPages={Math.max(1, Math.ceil(ledgers.length / rowsPerPage))}
          onNextPage={() => setCurrentPage((pageNo) => pageNo + 1)}
          onPageChange={setCurrentPage}
          onPreviousPage={() => setCurrentPage((pageNo) => Math.max(1, pageNo - 1))}
  ...
  if (view.mode === "vouchers") {
    ...
    return (
      <WorkspacePage
        title="Vouchers"
        description="Balanced double-entry vouchers from billing postings and manual accounts entries."
        actions={
          <NewButton label="New voucher" onClick={() => setView({ mode: "voucher-upsert" })} />
        }
      >
        <WorkspaceFilters
          searchPlaceholder="Search voucher, source document, type, or status"
          searchValue={search}
          onSearchValueChange={(value) => {
            setSearch(value);
            setCurrentPage(1);
        ...
        />
        <VouchersList
          vouchers={pageRows(vouchers, currentPage, rowsPerPage)}
          loading={vouchersQuery.isLoading}
          onNew={() => setView({ mode: "voucher-upsert" })}
        />
        <WorkspacePagination
          page={currentPage}
          rowsPerPage={rowsPerPage}
          showingLabel={buildShowingLabel(currentPage, rowsPerPage, vouchers.length)}
          singularLabel="voucher"
          totalCount={vouchers.length}
          totalPages={Math.max(1, Math.ceil(vouchers.length / rowsPerPage))}
          onNextPage={() => setCurrentPage((pageNo) => pageNo + 1)}
          onPageChange={setCurrentPage}
          onPreviousPage={() => setCurrentPage((pageNo) => Math.max(1, pageNo - 1))}
          onRowsPerPageChange={setRowsPerPage}
  ...
  if (view.mode === "reports") {
    return (
      <WorkspacePage
        title="Reports"
        description="Accounts reports generated from ledger and voucher postings."
      >
        <AccountsReportsWorkspace
  ...
  return (
    <AccountsOverview
      onOpen={(mode) => setView({ mode })}
      ledgers={ledgersQuery.data ?? []}
      vouchers={vouchersQuery.data ?? []}
      refreshing={ledgersQuery.isFetching || vouchersQuery.isFetching}
      onRefresh={() => {
        void ledgersQuery.refetch();
...
function AccountsOverview({
  ...
  ledgers: Ledger[];
  onOpen: (mode: "ledgers" | "vouchers") => void;
  onRefresh: () => void;
  refreshing: boolean;
  ...
  return (
    <section className="space-y-5">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card
          title="Ledgers"
          description="Create account ledgers and inspect debit, credit, and closing balances."
        >
          <div className="flex items-center justify-between gap-3">
            <StatusBadge tone="green">Double entry ready</StatusBadge>
            <Button type="button" onClick={() => onOpen("ledgers")}>
        ...
        <Card
          title="Vouchers"
          description="Review billing postings and create manual balanced vouchers."
        >
          <div className="flex items-center justify-between gap-3">
            <StatusBadge tone="blue">Tally ready</StatusBadge>
            <Button type="button" onClick={() => onOpen("vouchers")}>
              <Plus className="size-4" />
              Open
            </Button>
          </div>
