...
export function LedgerForm({
  ...
  saveError?: string;
  onBack: () => void;
  onSave: (payload: LedgerSavePayload) => void;
}) {
  ...
  return (
    <WorkspaceUpsertPage
      ...
      description="Create ledgers for double-entry vouchers and Tally-ready posting."
      onBack={onBack}
    >
      <form
        noValidate
      ...
      >
        <WorkspaceFormPanel title="Ledger details">
          {[...errors, ...(saveError ? [saveError] : [])].length ? (
            <WorkspaceFormBanner title="Unable to save">
              {[...errors, ...(saveError ? [saveError] : [])].join(" ")}
            </WorkspaceFormBanner>
          ) : null}
          <WorkspaceFormGrid>
            ...
            <WorkspaceFormField label="Group" required>
              <WorkspaceSelect
                ...
                onValueChange={(groupId) => setForm((value) => ({ ...value, groupId }))}
              />
            </WorkspaceFormField>
            <WorkspaceFormField label="Classification" required>
              <WorkspaceSelect
                ...
                onValueChange={(classification) =>
                  setForm((value) => ({
                    ...value,
                    classification: classification as LedgerClassification
                  }))
                }
              />
            </WorkspaceFormField>
            ...
            <WorkspaceFormField label="Status">
              <WorkspaceSelect
                ...
                onValueChange={(status) =>
                  setForm((value) => ({ ...value, status: status as "active" | "inactive" }))
                }
              />
            </WorkspaceFormField>
            <WorkspaceFormField label="Tally ledger name">
              <Input
                value={form.tallyLedgerName ?? ""}
                onChange={(event) =>
                  setForm((value) => ({ ...value, tallyLedgerName: event.target.value }))
                }
              />
            </WorkspaceFormField>
          </WorkspaceFormGrid>
...
export function VoucherForm({
  ...
  saveError?: string;
  onBack: () => void;
  onSave: (payload: VoucherSavePayload) => void;
}) {
  const [submitted, setSubmitted] = useState(false);
  ...
  const balanced =
    form.lines.reduce(
      (sum, line) => sum + (line.dc === "debit" ? line.amount : -line.amount),
      0
    ) === 0;
  const errors =
    submitted && (!result.success || !balanced)
      ? [
          ...(!result.success ? result.error.issues.map((issue) => issue.message) : []),
          ...(!balanced ? ["Debit and credit totals must match."] : [])
        ]
      : [];
  ...
  return (
    <WorkspaceUpsertPage
      title="New Voucher"
      description="Manual double-entry voucher. Billing postings are created through the backend posting contract."
      onBack={onBack}
    >
      <form
        noValidate
        onSubmit={(event) => {
          event.preventDefault();
          setSubmitted(true);
          if (result.success && balanced) onSave(result.data);
        }}
      >
          ...
          <WorkspaceFormGrid>
            <WorkspaceFormField label="Date" required>
              <Input
                ...
                onChange={(event) =>
                  setForm((value) => ({ ...value, voucherDate: event.target.value }))
                }
              />
            </WorkspaceFormField>
            <WorkspaceFormField label="Voucher type" required>
              <WorkspaceSelect
                ...
                onValueChange={(voucherType) =>
                  setForm((value) => ({
                    ...value,
                    voucherType: voucherType as VoucherSavePayload["voucherType"]
                  }))
                }
              />
            </WorkspaceFormField>
            <WorkspaceFormField className="md:col-span-2" label="Narration">
              <Input
                value={form.narration ?? ""}
                onChange={(event) =>
                  setForm((value) => ({ ...value, narration: event.target.value }))
                }
              />
            </WorkspaceFormField>
          </WorkspaceFormGrid>
        ...
        <WorkspaceFormPanel title="Lines">
          <div className="space-y-3">
            {form.lines.map((line, index) => (
              <div
                className="grid gap-3 rounded-md border p-3 md:grid-cols-[1fr_9rem_10rem]"
                key={index}
              >
                <WorkspaceFormField label="Ledger" required>
                  <WorkspaceSelect
                    value={line.ledgerId}
                    options={ledgers.map((ledger) => ({ label: ledger.name, value: ledger.id }))}
                    onValueChange={(ledgerId) => updateLine(index, { ledgerId })}
                  />
                </WorkspaceFormField>
                <WorkspaceFormField label="Debit/Credit" required>
                  <WorkspaceSelect
                    value={line.dc}
                    options={[
                    ...
                    onValueChange={(dc) => updateLine(index, { dc: dc as "debit" | "credit" })}
                  />
                </WorkspaceFormField>
