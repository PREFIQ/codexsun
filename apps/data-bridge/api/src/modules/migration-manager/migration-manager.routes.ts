...
export async function registerMigrationManagerRoutes(app: FastifyInstance, database: CompatibleDbPool) {
  const repository = new MigrationManagerRepository(database);
  ...
  app.post("/data-bridge/migration-jobs", async (request, reply) => {
    ...
    try { return reply.code(201).send({ data: await repository.create(parsed.data) }); }
    catch (error) { return reply.code(409).send({ message: error instanceof Error && error.message.includes("Duplicate") ? "Job name already exists." : "Could not save migration job." }); }
  });
