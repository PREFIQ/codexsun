...
export async function registerMigrationManagerRoutes(app: FastifyInstance) {
  ...
  app.post("/data-bridge/migration-jobs", async (request, reply) => {
    ...
    if (!parsed.success || !parsed.data.source.password || !parsed.data.target.password)
      return reply.code(400).send({ message: "Complete all required database settings." });
    const duplicate = (await repository.list()).some(
      (job) => String(job.name).toLowerCase() === parsed.data.name.toLowerCase()
    );
  ...
  app.post("/data-bridge/migration-jobs/:id/smoke-test/:side", async (request, reply) => {
    ...
    if (side !== "source" && side !== "target")
      return reply.code(400).send({ message: "Invalid database side." });
    ...
    try {
      const connection = await createDatabaseConnector(config).connect({
